...
import capture_training_backward_deferred as entry
import nan_backward_training_deferred as monitor


class DeferredEntryTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_environment_records_all42_endpoints_uncapped_and_dense_phases(self):
        ...
        self.assertTrue(recorded["deferred_scalar_trace"])
        self.assertFalse(recorded["raw_snapshot_present"])
    ...
    def test_loop_uses_local_optimizer_proxy_and_restores_forward_at_bound(self):
        ...
        with patch.object(monitor, "DeferredAllLayersTrainingBackwardProbe", FakeProbe), \
    ...
    def test_worker_bootstraps_before_runtime_imports_and_restores_loop(self):
        ...
        with patch.dict(os.environ, environment), patch.dict(sys.modules, {"gin": gin, bootstrap.__name__: bootstrap}), \
            ...
            entry.diagnostic_worker(0, 1, 0, 1, "localhost", "29885", "test.gin", "streaming-train-eval")
