...
DATA = Path(os.environ.get('LINEAR_DW_NATIVE_TEST_DATA_ROOT', '/data/mlperf_dlrm_v4/root_cause_20260919'))
ARTIFACTS = DATA / 'DW_native_override_artifacts_v1'
ORIGINAL_SHA = '1c7b790600e7dffdd4caaca8c1213c5480a81314ad3c13682eeb24335b8926f3'
DERIVED_SHA = 'd57129bfbd851f78ba148a2af82d0379704e3e3c2e5ef85190f8b4ecc8091a33'
...
static void check_buffer(void**p,void**e){assert(!p&&e&&e[0]==(H)1&&e[2]==(H)2&&e[4]==(H)3);assert(*static_cast<size_t*>(e[3])==172);
 auto*b=static_cast<unsigned char*>(e[1]);for(size_t i=0;i<172;++i)assert(b[i]==static_cast<unsigned char>(i*7+3));}
extern "C" int hipModuleLaunchKernel(H f,unsigned gx,unsigned gy,unsigned gz,unsigned bx,unsigned by,unsigned bz,unsigned sh,H st,void**p,void**e){
 event("launch");assert(f==(std::getenv("LINEAR_DW_NATIVE_OVERRIDE")?(H)0x5678:(H)0x1234));assert(gx==2&&gy==3&&gz==4&&bx==5&&by==6&&bz==7&&sh==8&&st==(H)0xab);check_buffer(p,e);errno=EDOM;return 17;}
extern "C" int hipExtModuleLaunchKernel(H f,uint32_t gx,uint32_t gy,uint32_t gz,uint32_t bx,uint32_t by,uint32_t bz,size_t sh,H st,void**p,void**e,H start,H stop,uint32_t flags){
 event("ext_launch");assert(f==(std::getenv("LINEAR_DW_NATIVE_OVERRIDE")?(H)0x5678:(H)0x1234));assert(gx==20&&gy==30&&gz==40&&bx==50&&by==60&&bz==70&&sh==((size_t(1)<<32)+8)&&st==(H)0xab&&start==(H)0xcc&&stop==(H)0xdd&&flags==11);check_buffer(p,e);errno=ERANGE;return 23;}
...
int main(int argc,char**argv){assert(argc==2);std::string test=argv[1],target=std::getenv("MOCK_TARGET");H f=nullptr;
 ...
 if(test=="env_changed")setenv("LINEAR_DW_NATIVE_OVERRIDE","b_reuse_off",1);
 ...
 unsigned char b[172];for(size_t i=0;i<sizeof(b);++i)b[i]=i*7+3;size_t size=sizeof(b);void*extra[]={(H)1,b,(H)2,&size,(H)3};
...
class NativeOverrideTests(unittest.TestCase):
    def test_all_CPU_cases(self):
        source = Path(__file__).with_name('replace_linear_dw_native_elf_v1.cpp')
        frozen = source.with_name('trace_projection_hip_launches_v2.cpp')
        metadata_path = ARTIFACTS / 'DW_kernel_metadata.json'
        self.assertEqual(hashlib.sha256(metadata_path.read_bytes()).hexdigest(), 'b8ee1f792b59a5d3aa4da70e005387cf65d138589e8f246826562082cf6e32ab')
        metadata = json.loads(metadata_path.read_text())
        target = metadata['.name']
        self.assertEqual((metadata['.kernarg_segment_size'], max(a['.offset'] + a['.size'] for a in metadata['.args'])), (176, 172))
        ...
        original = ARTIFACTS / 'DW_original.elf'
        derived = ARTIFACTS / 'DW_B_reuse_off.elf'
        ...
        keep = os.environ.get('LINEAR_DW_NATIVE_OVERRIDE_TEST_DIR')
        ...
        commands = [
            ['g++', '-shared', '-fPIC', '-O2', '-std=c++17', '-Wall', '-Wextra', '-Werror', str(source), '-ldl', '-pthread', '-lcrypto', '-o', str(root / 'replace_linear_dw_native_elf_v1.so')],
        ...
        def run(case, *, mode='original', path=original, expected=0, changes=None, test=None):
            ...
            env.update(LD_PRELOAD=str(root / 'replace_linear_dw_native_elf_v1.so'),
                LINEAR_DW_NATIVE_OVERRIDE=mode, LINEAR_DW_NATIVE_OVERRIDE_ELF=str(path),
        ...
        for mode, path, sha in [('original', original, ORIGINAL_SHA), ('b_reuse_off', derived, DERIVED_SHA)]:
        ...
        records, calls = run('disabled', changes={'LINEAR_DW_NATIVE_OVERRIDE':None, 'LINEAR_DW_NATIVE_OVERRIDE_ELF':None})
        ...
        for case, path in [('bad_hash', wrong), ('wrong_arm', derived), ('missing_file', root / 'absent.elf')]:
            ...
            self.assertEqual(calls.count('get_original'), 1)
            self.assertNotIn('load', calls)
        _, calls = run('wrong_arm_reverse', mode='b_reuse_off', path=original, expected=125)
        self.assertNotIn('load', calls)
        small = root / 'truncated.elf'; small.write_bytes(b'\x7fELF')
        _, calls = run('wrong_size', path=small, expected=125)
        self.assertNotIn('load', calls)
        for case, changes in [
            ('bad_mode', {'LINEAR_DW_NATIVE_OVERRIDE':'typo'}),
            ('missing_mode', {'LINEAR_DW_NATIVE_OVERRIDE':None}),
            ('missing_path', {'LINEAR_DW_NATIVE_OVERRIDE_ELF':None}),
            ('missing_trace', {'PROJECTION_HIP_LAUNCH_TRACE':None}),
            ('missing_zero', {'AMDGCN_USE_BUFFER_OPS':None}),
            ('nonzero_autotune', {'TRITON_FULL_AUTOTUNE':'1'}),
            ('nonzero_pipeline', {'TRITON_ALLOW_PIPELINING':'1'}),
            ('projection_arm_spelling', {'LINEAR_DW_NATIVE_OVERRIDE':'a_reuse_off'}),
        ]:
        ...
        self.assertEqual(hashlib.sha256(original.read_bytes()).hexdigest(), ORIGINAL_SHA)
        self.assertEqual(hashlib.sha256(derived.read_bytes()).hexdigest(), DERIVED_SHA)
        report = {'format':'native_linear_DW_override_CPU_tests_v1','status':'PASS','GPU_initialized':False,
                  ...
                  'original_ELF_sha256':ORIGINAL_SHA,'derived_ELF_sha256':DERIVED_SHA,
                  'DW_symbol':target,'declared_argument_bytes':172,'aligned_kernarg_segment_bytes':176,
                  'compiled_sha256':hashlib.sha256((root/'replace_linear_dw_native_elf_v1.so').read_bytes()).hexdigest(),
