...
**The two `nan` rows are nondeterministic**, so they carry run counts rather than
a step number. Measured 2026-09-04, batch 8 clamped is the only configuration
with no `nan` at all: 0 in 4000 steps, against 3 of 4 runs at batch 128 and 2 of
3 unclamped at batch 8. Both changes appear to raise the *rate* of one
nondeterministic corruption rather than switch it on — which matches [3]'s
original signature, an onset that moved from step 50 to step 2200 across compiler
versions. **A single clean run therefore clears nothing here**, which is the main
trap for anyone re-testing.

| Change | Symptom | Defect | Standalone reproducer for triage |
|---|---|---|---|
| **`BATCH_SIZE=128`** | `train_loss` **`nan`, first logged point (step 50), in 3 of 4 runs** and never recovers; the 4th stayed finite for 80 steps. PyTorch kernels at the same step and batch: finite, 3 of 3 | [3] | **None, and three probes actively failed to find one** — see [Still open](#still-open). E2e only, but now ~50 steps instead of 2200, so a hypothesis costs minutes. `DEBUG_NAN_HOOKS=1` does *not* name a producer: by the first non-finite forward 80 embedding tables are already poisoned, so the bad gradient came from outside the hooked modules |
| `TRITON_ALLOW_PIPELINING=1` | `nan` **within 300 steps in 2 of 3 runs** at batch 8 (step 100, step 300, none), against clamped controls finite in 2 of 2 plus the 4000-step run. No fault, no garbage | [4] | **None on this stack**, and this is the worst gap. [`repro_gfx1250_pipeliner.py`](../scripts/repro_gfx1250_pipeliner.py) is torch+triton only but calls `tl.make_block_ptr` in 6 places, so on `7ff97e3109` it dies with [5]'s removal error before testing anything; and `attn_check.py` now **passes** at `num_stages=2`, so attention is no longer a witness. Owed: a plain-pointer rewrite of the probe, aimed at a *non-attention* kernel (see below) |
| `AMDGCN_USE_BUFFER_OPS=1` | **Node hang, reboot required** | [1] | **Yes — the one to hand over.** `python scripts/repro_gfx1250_buffer_ops.py`: torch+triton only, no repo imports, no fbgemm, no dataset. `--compile-only` never dispatches, so triage can inspect the hybrid 32/64-bit addressing **without risking a node**, and it still reproduces on `7ff97e3109` |
| `TRITON_FULL_AUTOTUNE=1`, or reverting the pin to `BLOCK_N=128` | `Memory access fault … Page not present` within a few hundred batches; recoverable | [6] | **Yes, at two levels.** `python scripts/repro_gfx1250_attn_bwd.py --part bwd --layout contiguous` faults in ~15 s (needs the repo, no dataset), with `--part fwd`, `--layout guarded` and `--contextual 0 --no-targets` as controls. For the compiler team there is a checked **MIR reproducer** in [#3](https://github.com/raikonenfnu/training/issues/3), which is the level the fix has to land at |
