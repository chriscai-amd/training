...
one, and the `nan` defect [3] is **not** cleared by it — as of 2026-09-04 [3] is
confirmed still live and **the e2e result holds only at batch 8**: at batch 128
`train_loss` is `nan` within 50 steps. And the remaining
...
**Current stack — start here**

- [The working stack](#the-working-stack) — the recipe that trains end to end.
  - [Pins](#pins) · [Build and run](#build-and-run) ·
    [What the e2e run produced](#what-the-e2e-run-produced) ·
    [What the run does not establish](#what-the-run-does-not-establish)
- [Workarounds and fixes applied since master](#workarounds-and-fixes-applied-since-master)
  — every change against `mlcommons/training @ b8b3adc`, with motivation,
  downside and follow-up.
  - [Root cause of the attention-backward fault ([6] / [4d])](#root-cause-of-the-attention-backward-fault-6--4d)
- [Still open](#still-open) — what is unfixed, unmeasured, or owed a control.
  Re-tested 2026-09-04; read this before trusting the e2e result.
- [Defect record for the current stack](#defect-record-for-the-current-stack) —
  the one defect that survives on `7ff97e3109`, with its isolation evidence.
- [Reproducers](#reproducers) — the standalone scripts, and which are safe to
  run on a machine you cannot reboot.
- [API changes](#api-changes) — what upstream Triton removed and what replaced
  it.
  - [What upstream removed](#what-upstream-removed) ·
    [What we replaced them with](#what-we-replaced-them-with) ·
    [Files](#files) · [Compatibility](#compatibility)

**Reference**

- [Historical: defects by Triton version](#historical-defects-by-triton-version)
  — what breaks on 3.6.0, AMD 3.7.1 and AMD 3.8.0. No longer the recommended
  path; kept for provenance.
  - [Images tested](#images-tested) · [The table](#the-table) ·
    [Row 7 follow-up: coexec scheduler root cause](#row-7-follow-up-coexec-scheduler-root-cause)
- [The node](#the-node) — hardware, firmware and container inventory.
- [Getting it to run](#getting-it-to-run) — the non-defect setup work: dataset
  cache, table shrink, wave32.
- [Performance](#performance) — measured throughput and how it was keyed.
- [Retracted](#retracted) — claims made earlier in bring-up that did not hold.
- [Change inventory](#change-inventory) — file-by-file list of what this branch
  touches.
...
- **It does not clear [3], the `nan` — and [3] is now confirmed live.** The owed
  batch-128 run was made on 2026-09-04 and goes `nan` within 50 steps, so **this
  result is specific to batch 8**. See [Still open](#still-open).
...
- **Performance is unmeasured on this stack.** Of the two workarounds that trade
  throughput for correctness, row 2 now has a kernel-level number — the pinned
  tile is 1.54× slower on attention forward+backward — while row 3's e2e cost
  remains unquantified. Neither has been measured as an e2e throughput delta.
...
load at all on wave32; the rest are harness, measurement and diagnostics. Two of
them, rows 5 and 15, are **Triton-language API changes** made to accommodate
current upstream rather than to dodge a miscompile — [API
changes](#api-changes) has the before/after mapping, the call-site counts and the
one correctness pitfall the port introduces.
...
| 15 | **`&` / `\|` instead of Python `and` / `or` on masks** — `ops/triton/triton_position.py`. The other Triton-language change besides row 5; see [API changes](#api-changes) | Not compile-blocking today — the kernels already built — but Triton **warns and will error** on the Python operators applied to non-scalar tensors inside `@triton.jit`, so this is the second API accommodation for current upstream | None. For boolean masks the two are equivalent: 3.6.0's `visit_BoolOp` already lowered `and`/`or` to `logical_and`/`logical_or` | None — and note the "this left loads unmasked" reading of it was **retracted**; see [Retracted](#retracted) |
...
Re-tested on the working stack on 2026-09-04; results are folded in below and
the raw logs are under `results/`. The batch-8 e2e is unaffected by all of it.

- **[3] `train_loss=nan` — now reproducible, and it is a batch-size effect.**
  The owed batch-128 run was made (`results/nan_probe_b128/`,
  `AMDGCN_USE_BUFFER_OPS=0`, 3000 steps requested, stopped at 326,400 samples
  once the answer was in). `train_loss` is **`nan` from the first logged point,
  step 50** — not step 2200 as on AMD 3.7.1. Two shorter batch-128 runs agree,
  and all four batch-8 runs are clean, so the split is by batch size, not by
  step count: **8 → finite for 4000 steps, 128 → `nan` within 50.** That is a
  ~50-step reproducer where there was none. It also means the e2e result in
  [The working stack](#the-working-stack) holds *only* at batch 8. Not yet
  bisected to a kernel, and the interaction with pipelining below is untested.
- **[4] pipelining is *not* fixed upstream — it changed symptom.** With
  `TRITON_ALLOW_PIPELINING=1` on `7ff97e3109` the attention forward goes back to
  `num_stages=2` (confirmed on the decorated kernel, not just the config
  builder) and `scripts/attn_check.py` **passes all eight cases** — where AMD
  3.8.0 returned `3.3e+28`. But the e2e regresses: a 300-step batch-8 run goes
  `nan` at step 100, against a matched control that stays finite
  (`results/pipelining_probe/` vs `results/pipelining_control/`). No fault, no
  garbage — silent `nan`. **Row 3's clamp is still load-bearing**, and the
  attention-only check would have wrongly cleared it.
  - **This is a lead on [3].** The clamp only applies on Triton ≥ 3.8, so AMD
    3.7.1 — where [3] was first seen — ran *unclamped*. Pipelining producing
    `nan` is the same symptom from the same era. Worth testing whether [3] at
    batch 128 is pipelining-related, though the clamp is already on in the
    batch-128 run above, so if they are the same defect the clamp does not
    cover the batch-128 path.
- **[2] silent wrong results no longer reproduces.** `position_test`'s
  `test_add_timestamp_positional_embeddings_triton` **passes with
  `AMDGCN_USE_BUFFER_OPS=1`** on `7ff97e3109`, where it used to report 20/7640
  wrong elements at max abs diff 1.38. One kernel, so this does not clear the
  class — but the one case that demonstrated [2] is gone.
- **[4c] fused RNG stays fixed.** `scripts/repro_gfx1250_fused_rng.py` →
  `backward: PASS`. Row 4's gate is the best candidate for removal, pending a
  check that no other separated-RNG caller depends on it.
- **[1] buffer-op hang: codegen precondition unchanged.**
  `repro_gfx1250_buffer_ops.py --compile-only` still emits hybrid addressing on
  `7ff97e3109` — `JaggedIn` (4.064 GiB) and `OutA` (4.062 GiB) stay 64-bit while
  the small args narrow, giving 16 `buffer_store_b128` + 8 `global_load_b128` +
  8 `global_store_b128` in one kernel. The knob stays load-bearing. **[1b]'s
  matched control is still owed and is the one item that cannot be run here** —
  it requires dispatching with buffer ops on, which wedges the node.
...
- **Row 2's perf cost, now measured: the pinned tile is 1.54× slower.**
  `scripts/bench_attn_bwd_blockn.py` at a training-scale shape (8×8 heads,
  13,809 tokens, `attn_dim=hidden_dim=128`) times attention forward+backward at
  **11.22 ms at `BLOCK_N=64` against 7.30 ms at `BLOCK_N=128`**, reproducible to
  ±0.05 ms across repeats. VGPRs are 654 vs 987, **no spills in either** — more
  evidence spilling is not what [6] is about. This is a kernel-level number, not
  an e2e one; the e2e A/B is unavailable because `BLOCK_N=128` is the faulting
  config.
- **Row 3's perf cost is not measurable at batch 8**: 87.9 s with pipelining
  against 87.5 s clamped over 300 steps, but that comparison is startup-
  dominated and should not be quoted as the steady-state cost.
- **An intermittent batch-128 fault, seen once.** The first batch-128 launch
  died with `Memory access fault … on address 0xa4699000. Reason: Page not
  present` at global step 0, with buffer ops off. It did not recur across three
  later batch-128 runs, and it did not reproduce under
  `AMD_SERIALIZE_KERNEL=3` + `HIP_LAUNCH_BLOCKING=1` — an async signature. The
  process wedged after the fault but `docker kill` freed it and the GPU came
  back at 0% without a reboot. Unattributed; needs a repeat count before it can
  be called a defect.
...
- **To time a tile choice, use
  [`scripts/bench_attn_bwd_blockn.py`](../scripts/bench_attn_bwd_blockn.py)**
  (`--block-n 64|128`), one `BLOCK_N` per process so a fault in the 128 case
  cannot take the 64 measurement with it. It asserts that `_hstu_attn_bwd`
  actually launched and prints the compiled VGPR count, because the easy failure
  here is silently timing the PyTorch reference path: `hstu_mha` needs
