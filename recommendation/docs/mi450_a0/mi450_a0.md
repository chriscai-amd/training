...
Healthy replay restored all **150,042,474,496 embedding bytes** exactly and
matched forward output bytes. The preceding backward transition showed small
rounding differences, so exact determinism is measured rather than assumed.
Direct replay of the actual step-101 failure then hit a **GPU/MES hang at
06:25:50 UTC**, before a numerical replay verdict. Neither a culprit nor a fix
is established. A new probe captures extreme finite gradients as well as NaN/Inf
around the layer-1 input-gradient GEMM and layer-norm backward.

At the latest read-only check, the replay had exited **143** after SIGTERM and
VRAM had drained to **0.2 GB**, but the monitor still showed **100% GFX / ~858 W**.
Low VRAM does not establish recovered compute health; no post-hang compute check
or new replay has completed. Recovery and a successful stack check precede the
next GPU experiment.

Start with the [experiment record](#27-full-state-capture-and-replay-2026-09-18),
[backup plan](#28-capture-backup-and-retention), and
[detailed replay report](nan_replay_20260918.md). Historical early NaNs on the
stock Triton image, the `START_TS=150` NaNs, and the step-211 hang remain evidence;
...
| Direct replay of actual failure | No completed attempt | MES hang at 06:25:50; stopped, exit 143; no numerical replay verdict | `~/mi450_logs/nan_replay_long_20260918_0520/replay_hang_0630/` |
...
2. **Recover and verify GPU compute health.** At 06:57 UTC VRAM had drained to
   0.2 GB, but GFX/power remained 100%/~858 W after the MES failure. A monitor
   sample or an exited process does not establish working kernels. Recovery
   affecting this shared host requires user coordination. Keep the paired image,
   `AMDGCN_USE_BUFFER_OPS=0`, BLOCK_N=64 and pipeline clamp unchanged.
3. **Replay captured step 101 with the backward-boundary probe.** Inspect the
   layer-1 UVQK projection backward and input layer norm for NaN/Inf and finite
   magnitudes above `1e20`; save complete operation inputs before execution.
   The [prepared command](nan_replay_20260918.md#extreme-finite-gradients-and-next-replay)
   retains exact model-source checks. The probe's GPU behavior remains untested.
...
are in [nan_replay_20260918.md](nan_replay_20260918.md).
...
Healthy restoration is verified, but backward is not fully deterministic and
the actual failure replay hung before a verdict. A repeatable one-step NaN and
then an operation-level reproducer remain experimental results to obtain.
...
| `nan_backward_boundaries.py` | CPU checks passed; GPU validation pending | Prepared diagnostic for extreme finite inputs/outputs as well as NaN/Inf |
...
exited 143, without a report. GPU memory drained by 06:57, but successful compute
recovery remains unverified. Progress persistence and SIGUSR1 Python stack dumps
have been added for the next replay. See the detailed report for the exact
command and limits; no new model-kernel change is claimed as a fix.
...
- `scripts/nan_backward_boundaries.py` — capture pristine inputs at the suspect
  ...
  NaN/Inf. CPU-checked; GPU validation pending recovery.
