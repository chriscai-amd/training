...
This does **not** make current Triton a drop-in upgrade for the full training
workload.  On the same `7ff97e3109` build, all eight cases in `attn_check.py`
fail at the first Triton forward because `tl.make_block_ptr` now raises
`NotImplementedError: Block pointers have been removed in favor of the tensor
descriptor API`.  The three HSTU Triton files contain 34
`tl.make_block_ptr`/`tl.advance` calls.  They must be ported to plain pointers or
the current descriptor API before an end-to-end latest-main run is possible;
the existing descriptor/TLX path is NVIDIA Hopper-specific and deliberately
disabled on HIP.  Until that port is complete, keep
