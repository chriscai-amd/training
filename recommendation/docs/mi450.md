...
**2026-09-04 it trains end to end on Triton kernels at batch 8**: 4000 batches,
`run_stop`, finite loss, an eval AUC. That took workarounds for six Triton/LLVM
defects ([1], [2], [4], [4c], [5], [6]), three fbgemm patches for wave32, and a
table shrink to fit one GPU. **None of it is in the benchmark's math.**

Two limits on that result. It holds **only at batch 8** — at 128 `train_loss` is
`nan` within 50 steps ([3], open). And [6] is *worked around, not fixed*: the
LLVM hazard is live, tracked in
...
reference converges to target across 60 runs; this run is 1/72 of the way to that
reference's *first* eval checkpoint. See
[Support level](#support-level-against-the-mi350-reference) before quoting it.

Branch `chcai/mi450_a0`. Companion to [`training_recipe.md`](./training_recipe.md)
and [`perf_opt.md`](./perf_opt.md). Per-version history for 3.6.0 / 3.7.1 / 3.8.0
is reference only, in [Historical](#historical-defects-by-triton-version).

## Sections

**Current stack — start here**

- [Support level, against the MI350 reference](#support-level-against-the-mi350-reference)
  — what this stack does and does not claim. Read before quoting the e2e result.
- [The working stack](#the-working-stack) — the recipe, plus what breaks it.
...
- [Workarounds and fixes applied since master](#workarounds-and-fixes-applied-since-master)
  — every change against `b8b3adc`, with motivation, downside and follow-up.
  - [Root cause of the attention-backward fault ([6] / [4d])](#root-cause-of-the-attention-backward-fault-6--4d)
- [Still open](#still-open) — unfixed, unmeasured, or owed a control. Re-tested
  2026-09-04; read before trusting the e2e result.
- [Defect record for the current stack](#defect-record-for-the-current-stack) —
  the one defect surviving on `7ff97e3109`, with its isolation evidence.
- [Reproducers](#reproducers) — the scripts, and which are safe on a machine you
  cannot reboot.
- [API changes](#api-changes) — what upstream Triton removed and what replaced it.
...
## Support level, against the MI350 reference
...
**single-configuration functional bring-up**: one config completes and its loss
matches the PyTorch-kernel reference. It makes **no quality claim**.

| | MI350 (RCP) | MI450 A0 today |
|---|---|---|
| Global batch | 8192 / 16384 / 32768 | **8** — 1024× below the smallest RCP point |
| Samples | 112M to reach 0.75 at gbs 32768 (69M at 8192) | **32,000** |
| Eval AUC | 0.7636–0.7875, 60/60 runs ≥ 0.75 | **0.5233** — chance |
| Seeds | 20 per batch size | 1 |

For scale: **this run is 1/72 of the way to MI350's *first* eval checkpoint**,
which itself reads `0.5010`. Against samples-to-target it is 1/3800. It is not a
short convergence run; it is shorter than the reference's first datapoint.

**The gap is blocked, not merely unfinished.** Closing it means raising batch
size and running longer, and both are where the open defects live — batch 128
`nan`s in 3 of 4 runs ([3], no standalone reproducer), and longer runs raise the
odds of the same nondeterministic corruption. Batch 8 × 4000 steps is close to
the largest box that currently works, in *both* dimensions, so the next milestone
is closing [3], not accruing machine time.
...
- **Stronger than "it didn't crash."** The full Triton op set compiles and
  dispatches, and `train_loss` is `0.10000` against the PyTorch-kernel
  reference's `0.1015` — numerics agree with the reference implementation at the
  working point.
...
**single-config e2e completes (MI450 today)** → benchmark-valid config, real
batch, multi-GPU → **convergence to target (MI350)** → competitive throughput.
...
**The working configuration.** Every value below is confirmed against the run
that produced the result, [`results/smoke_7ff97e_pr7/train.log`](../results/smoke_7ff97e_pr7/train.log),
not just from the command we believe we typed. Note
[`training_recipe.md`](./training_recipe.md) is the **MI350, 8-GPU** recipe
(batch 32 × world 8) and does **not** apply here.

| Knob | Value | Why it is load-bearing |
|---|---|---|
| `BATCH_SIZE` | **8** | The whole result. 128 `nan`s ([3]); see [What breaks it](#what-the-e2e-run-produced) |
...
Not set, and that matters: `TRITON_ALLOW_PIPELINING` (unset keeps [4]'s
`num_stages=1` clamp) and `TRITON_FULL_AUTOTUNE` (unset keeps the `BLOCK_N=64`
pin for [6]). Both are workarounds that apply themselves — setting either
reintroduces a defect.

[`scripts/build_triton_from_source.sh`](../scripts/build_triton_from_source.sh)
`pip install -e`s Triton into `/opt/triton-custom` **inside a container** and
produces no image, so: start a long-lived container, build Triton in it, train
in it.
...
**`AMDGCN_USE_BUFFER_OPS=0` is the only correctness flag you must pass**, and the
runner defaults it. Every other workaround is gated in code on arch and Triton
version, so it applies itself. `AMD_SERIALIZE_KERNEL=3` is optional and costs
throughput — leave it off outside debugging.
...
`scripts/bisect_hooks/gpu_cleanup.sh` clears them; check `rocm-smi --showmemuse`
reads 0 % first.

### What the e2e run produced

All recorded 2026-09-04 on the stack above; logs under `results/`.

**The working point is batch 8** — `EMBEDDING_ROW_SCALE=0.25`, `MAX_SEQ_LEN=4096`,
3 HSTU layers, 1 GPU (`results/smoke_7ff97e_pr7/`).
...
**What breaks it.** Each row is one change from the recipe above. The first is
the dangerous one: the other three are debug knobs you would leave alone, but
batch size is an ordinary configuration choice. The last column is what a
Triton-side triage can run — **only two of the four have one**.

Batch 8 clamped is the only configuration with no `nan` at all — 0 in 4000 steps.
Both `nan` rows below are nondeterministic and appear to raise the *rate* of one
corruption rather than switch it on, which matches [3]'s original signature: an
onset that moved from step 50 to step 2200 across compiler versions. **A single
clean run clears nothing**, which is the main trap for anyone re-testing.

| Change | Defect | Deterministic? | Symptom | Suspicious component | Standalone reproducer for triage |
|---|---|---|---|---|---|
| **`BATCH_SIZE=128`** | [3] | **No** — 3 of 4 runs | `train_loss` `nan` at the first logged point (step 50), never recovers. The 4th run stayed finite for 80 steps. PyTorch kernels at the same batch and step: finite, 3 of 3 | **A Triton HSTU op, and not attention.** TBE and the optimizer run in *both* arms and the PyTorch arm is finite, so neither is the sole producer. Attention in isolation is bit-exact at the true e2e shape. The module hooks only ever report `downstream`, so the producer is most likely inside a **fused autograd function that is not a module boundary** — `hstu_preprocess_and_attention` fits, and it is also [6]'s kernel. Chain: wrong gradient → TBE writes it into an embedding table → `nan` a few steps later | **ROOT-CAUSED at batch 1024 (2026-09-10): `_ln_mul_dropout_fwd_rng` int32 row-offset truncation — deterministic standalone reproducer at `scripts/repro_gfx1250_lnmuldropout_i32.py`, write-up in [`lnmuldropout_i32_handoff.md`](lnmuldropout_i32_handoff.md). The batch-128 `nan` on 3.6.0/3.7.1 is a separate, still-unexplained defect; for it there is still none, but the *codegen* half reproduces standalone: `scripts/repro_gfx1250_extvgpr_value.py --compile-only` builds issue #3's allocation (a lane-derived value in an extended VGPR live across a WMMA chain, 602 VGPRs, **zero spills**) in a torch+triton kernel with no repo dependency — see [`gfx1250_extvgpr_handoff.md`](gfx1250_extvgpr_handoff.md). **The value-victim hypothesis did not hold:** that kernel is bit-exact over 200 iterations × 6 repeats, so an extended-VGPR carrier alone does not produce wrong numbers. Also see [Still open](#still-open). E2e only, but ~50 steps instead of 2200, so a hypothesis costs minutes. `DEBUG_NAN_HOOKS=1` does *not* name a producer: 80 embedding tables are already poisoned by the first non-finite forward |
| `TRITON_ALLOW_PIPELINING=1` | [4] | **No** — 2 of 3 runs (step 100, step 300, none) | `nan` within 300 steps at batch 8, against clamped controls finite 2 of 2 plus the clean 4000-step run. No fault, no garbage | **The HIP software pipeliner at `num_stages ≥ 2`.** Attention is cleared at 2 (`attn_check.py` passes), so the witness is among the **other 26** kernels that pipeline unclamped: jagged concat/split and `jagged_dense_bmm` (3–5), layer-norm forward and `dwdb` backwards (3), the fused LN×mul×dropout kernels (3), position (2–4), swiglu (2–4), addmm (2). All are grid-strided loops over jagged extents | **None on this stack**, and this is the worst gap. [`repro_gfx1250_pipeliner.py`](../scripts/repro_gfx1250_pipeliner.py) is torch+triton only but calls `tl.make_block_ptr` in 6 places, so on `7ff97e3109` it dies with [5]'s removal error before testing anything. Owed: a plain-pointer rewrite aimed at a non-attention kernel, and it must survive a 2-in-3 hit rate to prove anything |
...
**Narrowing [4] for whoever writes that probe.** Unclamped, **28 autotuned
kernels** run at `num_stages ≥ 2` — attention and addmm at 2, and the jagged,
layer-norm, linear, position and swiglu kernels at 3–5. Attention is cleared
(`attn_check.py` passes at 2), so the witness is among the other 26, which
matches the AMD 3.8.0 finding that the jagged and layer-norm kernels faulted in
their own right. To list them, with or without `TRITON_ALLOW_PIPELINING=1`:
...
Seen once and unattributed: one batch-128 launch died at step 0 with `Memory
access fault … on address 0xa4699000`, with buffer ops off. It did not recur in
three later batch-128 runs and did not reproduce under `AMD_SERIALIZE_KERNEL=3`
— an async signature. Details in [Still open](#still-open).

### What the run does not establish

- **[3] is not cleared** — it is confirmed live at batch 128, above.
- **The AUC is not a quality signal.** `0.5233` after 32,000 samples is barely
  above chance; it says the eval path runs and returns a finite metric.
- **The model is shrunk.** `EMBEDDING_ROW_SCALE=0.25` (~140 GB) against a
  ~559 GiB fp32 reference, ids modulo-wrapped into the smaller vocabulary. Not a
  valid MLPerf configuration.
- **One GPU.** Nothing exercises sharding, and row 2's gate touches device-0
  initialisation at import time — exactly where a multi-rank run would notice.
- **The buffer-op hang class is avoided, not fixed**, and [1b] shows hangs that
  survive the knob.
- **E2e throughput is unmeasured.** Row 2 has a kernel-level cost (1.54×
  slower); neither row 2 nor row 3 has an e2e number.

## Workarounds and fixes applied since master

Against `mlcommons/training` @ `b8b3adc`, the commit this branched from. The
delta is 27 files; below is every change that is a **workaround or a fix**.
Scripts, docs and traces are excluded and listed under [Change
inventory](#change-inventory).

Rows 1–5 are what the e2e run depends on, 6–8 are what makes the image build on
wave32, the rest are harness and diagnostics. Rows 5 and 15 are **Triton-language
API changes** for current upstream rather than miscompile dodges; [API
changes](#api-changes) has the mapping and the one pitfall the port introduces.
Bracketed ids are stable — cite those, not row numbers.
...
Re-tested on the working stack 2026-09-04; logs under `results/`. Batch 8 is
unaffected by all of it.

**Live defects**

- **[3] `train_loss=nan` at batch 1024 — ROOT-CAUSED (2026-09-10).
  `_ln_mul_dropout_fwd_rng` truncates its output row offset to int32 and writes
  ~4.3 GB outside the destination tensor.** Full write-up and handoff:
  [`lnmuldropout_i32_handoff.md`](lnmuldropout_i32_handoff.md). Standalone
  reproducer (torch + triton only, deterministic, cannot fault):
  `AMDGCN_USE_BUFFER_OPS=0 python3 scripts/repro_gfx1250_lnmuldropout_i32.py --n 1399126 --canary-gib 2`
  - **The defect.** In `triton_hstu_linear.py:148`, the eight `Y` stores (:267,
    :272, :277, :283, :288, :294, :299, :305) and the `X`/`U` loads (:183, :189)
    index with a raw int32 `rows`, while the dropout-mask pointer three lines
    above (:231) *is* widened to int64 and every neighbouring kernel in the same
    file widens too (:114, :336-338, :490, :518). `stride_y = 3 * D = 1536` at
    the production shape (`D = 4 heads × 128 = 512`, `concat_u=concat_x=True`),
    so `rows * stride_y` overflows int32 at **row 1,398,102** and the store
    lands at **`Y − 4.29 GB`**. `row_mask = rows < N` does not help: it is
    computed on the unwrapped row id, so the wild store proceeds under a passing
    mask.
  - **Verified on hardware, exactly.** `N=1398101` (last in-range row) PASS;
    `N=1399126` FAIL with first bad row **1398102** — the predicted wrap row to
    the row — 1024 bad rows (exactly the rows over threshold) and **1,572,864 =
    1024 × 1536 canary elements clobbered in an unrelated live tensor, with zero
    faults.** `--fix` (one `rows.to(tl.int64)`) is clean at every N.
  - **Why it looks batch-size-gated.** The kernel's autotune key is
    `key=["BLOCK_D"]` — **codegen is identical at every batch size.** `N` is the
    total jagged rows in the batch, so batch size enters only by scaling `N` past
    1,398,102. The per-sample row count is set by `HISTORY_STRATEGY`, which the
    run leaves at its default `interleaved`, and `yambda_5b.gin:463-478` records
    a **measured** effective length per strategy: `interleaved` ~2663 events
    (`HISTORY_LENGTH=4086` is a *per-pool* cap there, and the like pool fills
    only ~105 of its 1362 slots with no reallocation), `last_n` ~4085.

    | strategy | events/sample | N @ 128 | N @ 1024 | wrapping @ 1024 | crossover |
    |---|---|---|---|---|---|
    | `interleaved` (**in use**) | ~2663 | 342 K | **2.74 M** | 1.34 M (48.9%) | batch 523 |
    | `last_n` | ~4085 | 524 K | 4.19 M | 2.79 M (66.7%) | batch 342 |

    **The verdict is identical under either strategy** — 128 sits below the wrap
    row with ~4× margin, 1024 sits above it by ~2× — so the conclusion does not
    depend on the one free parameter. (An earlier revision of this block said
    N ≈ 4.19 M at batch 1024; that assumed the nominal 4086-row fill and
    overstated `interleaved` by ~35%.) **Still an inference:** these are corpus
    statistics from the gin file, not `N` read out of the live kernel — the
    instrumented run wedged the node before printing.
  - **Why it is silent.** `Y` is a fresh `torch.empty` in a process already
    holding the ~140 GiB embedding table, so `Y − 4.29 GB` lands in other live
    allocations, not unmapped memory: no fault, just poisoned tensors — [3]'s
    exact recorded signature. It also explains the wandering onset (91 vs ~185
    with an identical `SEED=1` data stream): *which* tensor sits below `Y` is a
    per-process allocator accident.
  - **Why only here.** `hstu_compute.py` reaches this kernel only when
    `use_separated_rng_ln_mul_dropout()` is true (`ops/utils.py:100-165`), which
    on AMD means **gfx1250 + Triton ≥ 3.8** — a correctness workaround for [4c].
    MI300/A100/H100 take the legacy `_ln_mul_dropout_fwd`, whose row offsets are
    widened (:338). Arch-correlated without being an arch bug.
  - **This is NOT the batch-128 `nan` of 3.6.0/3.7.1.** On those versions the
    separated-RNG kernel is not on the path at all, and batch 128 never
    approaches the wrap row. Consistent with batch 128 now being clean 2/2 over
    4000 steps on `7ff97e3109` while batch 1024 fails: two sequentially exposed
    defects, which is why the "step 50 → step 2200 → never" trail never
    converged.
  - **Fix applied and CONFIRMED e2e (2026-09-10).** `rows_i64 = rows.to(tl.int64)`
    is defined once at `triton_hstu_linear.py:186` and used by both loads (:189,
    :194), the mask-pointer reuse (:236) and **all eight `Y` stores** (:272-:310);
    no raw `rows[:, None]` survives in the kernel. `Mean + rows` / `Rstd + rows`
    are left alone (element stride 1, cannot reach 2**31 before `N` does).
  - **Backward kernels audited — clear.** `_ln_mul_dropout_bwd_dwdb` (:922) and
    the Helion variant (:2711) do use `rows[:, None] * D`, but their `N` argument
    is `tile_num = max(1, min(sms * 64, N // 4))` (~16 K), four orders of
    magnitude below the wrap row. The `dx_du` backward kernels already widen
    everywhere (:523, :714-719, :2578-2583). **The forward was the only defect.**
  - **E2E confirmed (2026-09-10).** Same config that failed 2/2 at batch 1024,
    with the fix applied: **600/600 steps, 60 logged losses, zero non-finite,
    exit 0**, no fault or hang (`~/mi450_logs/b1024_fixed.log`). Loss flat in
    0.13744-0.14013, last 0.13826 @ 600. That is **3.2x past the later prior
    onset (~190) and 6.6x past the earlier (91)**. Contrast the unfixed run
    (`prod_b1024_smoke.log`): last finite 0.13981 @ 180, `nan` at 190 and 200.
    - `run_stop status="aborted"` on this run is **not** a failure signal — it
      is the MLPerf convergence verdict, and *every* run in this doc reports it,
      including the two clean 4000-step batch-128 runs. Likewise eval AUC 0.4994:
      614,400 samples of a 5B corpus with `dense_lr=2.5e-08` still inside a
      24,000-step warmup.
    - **Still n=1** against a nondeterministic onset. The onset migrated 44x once
      before (step 50 -> 2200) without being fixed, so a rarer second defect
      beyond 600 steps is not excluded. Confidence ~90%, not higher.
  - **Not yet done:** a second independent 600-step run at batch 1024 (two clean
    runs beat one longer run against a nondeterministic failure); measure `N`
    directly in the kernel; `--sweep-config` over all 15 autotune configs.
  - **Owner is this repo, not the LLVM/AMDGPU team.** The generated code
    faithfully implements what the kernel source asks for; the source asks for a
    32-bit multiply.
...
- **[4] pipelining is not fixed upstream — it changed symptom.** With
  ...
  the decorated kernel) and `attn_check.py` **passes all eight cases**, where
  AMD 3.8.0 returned `3.3e+28`. But e2e goes `nan` **in 2 of 3 runs within 300
  steps** at batch 8 — step 100, step 300, then none — against clamped controls
  finite in 2 of 2 plus the clean 4000-step run. **Row 3's clamp stays
  load-bearing**, and the attention-only check would have wrongly cleared it.
  - **Probably the same defect as [3].** Both are nondeterministic `nan` with a
    moving onset, and batch 8 clamped is the one configuration that never
    produces it. The clamp only applies on Triton ≥ 3.8, so AMD 3.7.1 — where
    [3] first appeared — ran unclamped. Reading them as one corruption whose
    exposure rises with pipelining *or* row count fits every run to date, and
    would mean the clamp lowers the rate without covering the batch-128 path.
- **[1] buffer-op hang: codegen precondition unchanged.** `--compile-only` still
  emits hybrid addressing — `JaggedIn` and `OutA` stay 64-bit while the small
  args narrow. Knob stays load-bearing. **[1b]'s matched control is owed and is
  the one item that cannot be run here**: it needs a dispatch with buffer ops
  on, which wedges the node.
...
- **An intermittent batch-128 fault, seen once**, at step 0 with buffer ops off
  (`Page not present`, address `0xa4699000`). No recurrence in three later runs,
  and none under `AMD_SERIALIZE_KERNEL=3` + `HIP_LAUNCH_BLOCKING=1`. The process
  wedged behind it but `docker kill` freed it and the GPU returned to 0% without
  a reboot. Needs a repeat count before it counts as a defect.

**Cleared on this stack** — candidates for removing a workaround

- **[2] silent wrong results no longer reproduces.** `position_test`'s
  `…_triton` passes with `AMDGCN_USE_BUFFER_OPS=1`, where it reported 20/7640
  wrong elements at max abs diff 1.38. One kernel, so the class is not cleared.
- **[4c] fused RNG stays fixed** (`backward: PASS`). Row 4's gate is the best
  removal candidate, pending a check that no other caller depends on it.
...
- **Row 3's cost is not measurable at batch 8**: 87.9 s against 87.5 s over 300
  steps, but that is startup-dominated and is not a steady-state number.
...
pick one with `IMG=`. `repro_gfx1250_buffer_ops.py` and
`repro_gfx1250_pipeliner.py` need **only torch and Triton**, so those are the
ones to hand over without this tree. `repro_gfx1250_fused_rng.py`,
...
- **E2e reproducers** need the yambda-5b cache under `DLRM_DATA_PATH` and go
...
**Reference only** — [the working stack](#the-working-stack) is `7ff97e3109` with
the fix set above. Kept for when you are handed one of these images, bisecting a
regression, or writing up a defect for the Triton team.
...
stack](#the-working-stack), rebuild Triton on top of `:gfx1250` rather than
...
**Embedding shrink** — `EMBEDDING_ROW_SCALE`: ~559 GiB of fp32 reference tables
against one ~432 GiB GPU.
...
fault](#root-cause-of-the-attention-backward-fault-6--4d).
