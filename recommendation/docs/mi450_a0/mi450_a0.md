...
Updated **2026-09-18**. The yambda-5b HSTU ranker runs on one
**MI450 A0 engineering sample**, `gfx1250`. The maintained test configuration is
**batch 1024, `START_TS=0`, seed 1, embedding row scale 0.25**. This uses the
benchmark's start point; the reduced embedding vocabulary still prevents an
MLPerf-valid accuracy claim.

**The NaN remains unresolved, and we now have a complete failure capture.**
After an AC cycle, four bounded runs completed 379 finite steps in total. A
longer uninstrumented run then produced NaN loss at **step 775**. The follow-up
recorder caught **38 nonfinite parameter gradients at step 101**, with finite
forward loss/predictions, and preserved the complete pre-step states for
**100 and 101**, including embedding tables and fused optimizer state.

Direct replay first hit a **GPU/MES hang at 06:25:50 UTC**. After a second
user-performed AC cycle at about **07:24 UTC**, the stack health check passed
and **four direct step-101 attempts completed with finite gradients and exact
forward outputs**. The first pair showed a **12,841× difference in the maximum
incoming `duvqk` gradient**, already before the layer-1 GEMM and layer norm.
The second pair verified all **150,042,474,496 embedding bytes**, small state,
controls and RNG on every repeat and saved complete selected-operation tensors;
all **12,043,722,752 GEMM input/output bytes matched** between those repeats.
Two isolated layer-norm replays also matched all captured output hashes. The
large finite divergence disappeared under the additional verification/persistence,
which changes execution history.
**Neither a culprit nor a fix is established.** Known numerical inputs can now
be replayed, but the original failure's execution conditions are not reproduced.

Shared trainer integration and host-specific controls are documented in
[the A0/B0 reconciliation guide](../mi450_a0_b0_reconciliation_20260918.md).
The shared attention register cap is opt-in: unset or `HSTU_BWD_MAX_VGPR=0`
preserves the A0 baseline described here.

Start with the [experiment record](#27-full-state-capture-and-replay-2026-09-18),
[backup plan](#28-capture-backup-and-retention),
[original replay report](nan_replay_20260918.md), and
[second-AC findings and resume commands](nan_replay_post_ac_20260918_0730.md).
Historical early NaNs on the stock Triton image, the `START_TS=150` NaNs, and
the step-211 hang remain evidence;
a common cause with the new failure is unproven. Batch 128, batch 8 and
`START_TS=150` are retained as historical defect experiments.
...
`ts=0`, the post-AC 319-step run was finite, but the subsequent longer run
failed at step 775 and the capture run failed during step-101 backward. See
...
2. [**The live nan**](#2-the-live-nan) — **the active investigation; start here**
   ...
   - [2.4 Instrumentation and harness](#24-instrumentation-and-harness) ·
     ...
     [**2.6 The ts=0 hang (2026-09-18)**](#26-the-ts0-hang-2026-09-18) — 211 finite
     steps, then an unkillable stall
   - [2.7 Full-state capture and replay](#27-full-state-capture-and-replay-2026-09-18) ·
     [2.8 Capture backup and retention](#28-capture-backup-and-retention)
3. [The working stack](#3-the-working-stack) — pins, configuration, commands
   - [3.1 Pins](#31-pins) ·
     [**3.1.1 The image/Triton pairing**](#311-the-imagetriton-pairing-and-how-to-build-it)
     — which image with which Triton, and how to build it so it survives
     `docker rm` ·
...
**`START_TS=0` is required for benchmark-aligned training.** All current
experiments use the pinned paired image in [3.1](#31-pins), batch 1024 and
embedding scale 0.25. The capture/replay experiments diagnose corruption; they
do not establish convergence or make the reduced-vocabulary model benchmark-valid.

| Experiment | Extent | Observed result | Evidence |
|---|---|---|---|
| Post-AC bounded training | 3 × 20 and 319 steps | 379/379 finite losses, exits 0; no MES/GPU fault | [Retest report](nan_post_ac_20260918.md) |
| Post-AC uninstrumented long run | First NaN at 775; stopped at 797 | NaN reproduced; no complete pre-step state | `~/mi450_logs/nan_long_20260918_0417/` |
| Healthy forced capture/replay | Step 3; direct and step 2 → 3 repeats | Exact restored bytes and forward outputs; small backward update differences | [Replay validation](nan_replay_20260918.md#replay-and-validation) |
| Full-state capture run | Stopped during step 101 | Finite forward; 38 nonfinite gradients before clipping; complete step-100/101 capture | [Step-101 capture](nan_replay_20260918.md#step-101-capture-and-replay-hang) |
| First direct replay of actual failure | No completed attempt | MES hang at 06:25:50; stopped, exit 143; no numerical replay verdict | `~/mi450_logs/nan_replay_long_20260918_0520/replay_hang_0630/` |
| Second-AC direct replays | Two pairs, four completed attempts | Exact forward and finite gradients; first pair had large pre-GEMM divergence, absent with full repeat verification and operation dumps; saved GEMM pairs byte-exact | [Second-AC report](nan_replay_post_ac_20260918_0730.md) |
| Isolated captured layer norm | Two fresh-input GPU attempts | All full-output hashes match capture; FP64 `d_x` reference matches BF16 cast on 16 control rows | [Operation validation](nan_replay_post_ac_20260918_0730.md#completed-replay-findings) |
| Earlier paired-image training | 211 finite steps | MES/virtual-memory hang; driver debug halt initially prevented termination | [Original hang](#26-the-ts0-hang-2026-09-18) |
| Earlier stock-Triton training | 20-step comparisons | NaN at steps 5–11 in 1 of 2 valid identical runs | [Early NaN](#21-the-ts0-nan-nondeterministic-onset-step-5-11) |

The bounded finite runs did not predict the longer-run outcome. The step-101
capture is the current diagnostic starting point: replay the saved inputs and
state, locate the first anomalous operation boundary, then reduce it to an
operation-level reproducer. Another finite soak run would not establish a fix.

The historical production-batch bring-up runs below used **`START_TS=150`**,
which skips early windows. Verified examples include `b1024_fixed`,
`b1024_fixed_rep2`, `b1024_rep2`, `prod_b1024_smoke`, `nan128/long1`,
`retest_0910/pipe_b1024_r1`, `smoke_7ff97e_pr7` and
`b1024_c07_i64fix_0916_2243`.
...
than step 1340. The later failures show that a 600-step finite run does not
establish that all numerical-corruption mechanisms are fixed.

**Related NaN symptoms were observed on both A0 and B0.** The historical
step-1340 failure has a similar **B0** observation at step **1370**
...
container, same Triton pin, same source. This motivates investigating the shared
software stack, but similar onset does not establish one cause or exclude
hardware-dependent behavior. The step-101 producer is still unproven. Full
historical comparison is in [2.2](#22-the-long-run-nan-at-step-1340-a0-and-1370-b0).
...
**Batch 1024 runs, but numerical stability remains unresolved.** The two
600-step bring-up runs used `START_TS=150`. At the required `START_TS=0`, the
paired post-AC stack completed the bounded series, then produced NaN loss at
step 775 and nonfinite backward gradients at step 101 in the capture run.
Earlier stock-Triton runs failed at steps 5–11. Those historical onset figures
do not describe the current stack; the captured failure now guides triage
([2](#2-the-live-nan)).
...
| The stack is clean | **No.** The paired stack reproduced NaN loss at step 775, nonfinite backward gradients at step 101, and an MES hang during replay. Earlier failures remain open; their common cause is unproven. The 379 finite post-AC steps and healthy replay validate bounded behavior and capture fidelity, not a fix. |
...
**The current investigation starts from the complete step-101 capture.**
The table separates observed failures without assuming how many underlying
defects they represent.

| Observation | Start point / stack | Onset | Current interpretation |
|---|---|---|---|
| [Full capture](#27-full-state-capture-and-replay-2026-09-18) | `START_TS=0`, paired image + `7ff97e3109` | Step 101, backward | Finite forward; layer-1 input norm is the first candidate parameter-gradient boundary; operation producer unproven |
| [Long follow-up](#27-full-state-capture-and-replay-2026-09-18) | Same start point / paired stack | NaN loss at 775 | Positive reproduction, without a complete replay state |
| [Replay hang](nan_replay_20260918.md#step-101-capture-and-replay-hang) | Saved step-101 state / paired stack | First attempted direct replay | MES failures and ROCr signal wait; no NaN replay verdict |
| [Second-AC replay](nan_replay_post_ac_20260918_0730.md) | Same saved step-101 state / paired stack | Four completed direct attempts | Finite; first pair differed greatly before layer-1 GEMM, while full-verify/save pair had matching maxima; execution-history dependence remains open |
| [Earlier hang](#26-the-ts0-hang-2026-09-18) | `START_TS=0`, paired stack | 211 finite steps | MES/virtual-address path; debug halt explained the initial unkillable loop |
| [Earlier NaN](#21-the-ts0-nan-nondeterministic-onset-step-5-11) | `START_TS=0`, stock Triton 3.6.0 | Steps 5–11 | Nondeterministic in the historical two-run comparison |
| [Historical long NaN](#22-the-long-run-nan-at-step-1340-a0-and-1370-b0) | `START_TS=150`, `7ff97e3109` | A0 1340 / B0 1370 | No established link to the current failure |

The paired image and Triton pin allow training but have not resolved either
numerical corruption or GPU hangs. Keep stack changes separate from component
replays so each result remains attributable to a known configuration.
...
The historical early-onset study below remains useful evidence. Its 1-in-2
hit rate and ~90-second cycle time do not describe the current paired stack.
The active next steps are [2.5](#25-what-runs-next), using the preserved
step-101 state rather than trying to reconstruct it from the older onset logs.
...
**Consequence for a handoff.** Wandering onset does not prevent an operation
reproducer once the failing inputs and state are captured. The old onset logs
alone cannot supply that state; the new step-101 artifact can be used to attempt
direct replay. A deterministic failing replay has not yet been demonstrated.
The target remains a named component and an independently replayable operation,
with hit rates reported if timing-dependent behavior persists.
...
Separate from the `nan`: the same transition appears in the short g01/i01/i02
series, including runs with finite loss throughout. The printed values vary
slightly between runs; one representative sequence is:

```
step 7   window_ne=2.4549       window_auc=0.5574
step 8   window_ne=6.7943e+11   window_auc=0.9912
step 9+  window_ne=6.79e+11     window_auc=0.5000   (in this short series)
```

At step 8 the reported normalized entropy gains eleven orders of magnitude
...
This is **misleading convergence reporting**, but the earlier claim that it
causes a spurious early termination was incorrect. Source review on 2026-09-18
shows that `MetricsLogger.compute_and_log` only latches a TensorBoard/log
time-to-target flag here. It neither stops training nor emits MLPerf
`run_stop`; streaming convergence decisions consume holdout evaluation results
separately. The later 0.5000 AUC and zero GAUC sample count are observations of
this short series, not proof that the accumulators are permanently corrupted.

**The large NE has a metric explanation.** TorchRec `ne.py` clamps the mean
label to `eta=1e-12`. For an all-negative window, normalized entropy is
`mean_BCE / -ln(1-eta)`, so ordinary BCE near 0.68 becomes NE near `6.8e11`.
The old step-10/11/12 captures have finite all-zero labels and unit weights.
In the fresh finite baseline, step-7/8 losses 0.13599 and 0.13577 give average
BCE 0.6794 after undoing the 0.2 causal weight; that predicts
**679,415,029,828.8**, matching the logged `6.7942e11` at print precision.
The observed NE jump therefore does not establish a corrupted accumulator or a
numerical precursor to the NaN.

**NE and AUC use different effective windows.** Both are configured for 2500
samples, but TorchRec's NE window buffer evicts whole batches, retaining the
last two 1024-sample batches. AUC trims individual samples and retains exactly
2500. At step 8, AUC can still include 452 samples from step 6 while NE uses
only steps 7–8. AUC explicitly returns 0.5 once its entire window has one class.
This explains why high AUC and a nearly zero NE denominator can coexist.
The exact step-8 AUC has not been replayed: the saved post-onset captures do not
contain its predictions. See [the source and capture review](nan_source_review_20260918.md)
for the measured tensors and installed-source details.

**State:** the NE magnitude is explained; training-window time-to-target
reporting remains misleading as convergence evidence. The exact AUC value is
not yet replayed, and neither metric observation identifies the NaN producer.
...
observed onset from step 11 to step 5, but those two runs alone cannot separate
an instrumentation effect from the nondeterminism subsequently established by
identical runs ([2.1.1](#211-nondeterminism-established-series-nan_onset_0917i-2026-09-17)).
Keep uninstrumented onset measurements separate from component attribution.
The repaired module probe avoids explicit device synchronization and GPU scalar
reads on its step path; its extra kernels, allocations, and full weight scans
can still change scheduling and the chance of reproducing the defect.
...
**`scripts/nan_module_probe.py` — repaired on 2026-09-18.** Enable with
`NAN_MODULE_PROBE=1`; add `NAN_MODULE_PROBE_PARAMS=1` to inspect direct parameters
and actual FBGEMM embedding weights. The report identifies an observed module
boundary and distinguishes bad inputs or existing bad weights from a candidate
forward producer. It does not identify the responsible kernel by itself.

- **State from the first bad call.** Paired pre/post hooks include keyword
  arguments. The pre-hook checks inputs and weights before the module runs;
  the post-hook latches that call's input, weight, and output status, step, and
  actual output order into device-resident int32 records. Later poisoned calls
  do not change the classification. A leaf with fully inspected finite inputs,
  non-finite output, and finite checked weights is reported as a **candidate**;
  weights are explicitly labeled unchecked when parameter checks are off.
- **Bad weights can trigger independently.** A corrupted unused embedding row
  is reported even if the current gather output is finite. The separate
  `weight_bad` and `output_bad` columns preserve that distinction. A bad weight
  observed before forward points to existing state corruption; it does not
  establish which earlier backward operation or update caused it.
- **Completion-gated host snapshots.** Only one pinned `non_blocking` D2H copy
  is in flight. A CUDA event recorded after the copy must return true from
  `query()` before the host reads or reuses its buffer. Reaching the next CPU
  iteration is not evidence that the copy completed. The trainer polls after
  ordinary metric logging and at the next step, without adding a wait for the
  event or reading a GPU scalar.
- **The real TorchRec compute path is traversed.** The probe walks raw
  registered `_modules` and explicitly follows the plain Python `_lookups`
  list in `ShardedEmbeddingCollection`. Ordinary module traversal misses that
  list: registered embedding parameter owners can exist without ever executing
  forward, while the native TBE under `_lookups` performs the lookup and update.
  TBE checks use `split_embedding_weights()` views of actual
  `weights_dev`/`weights_host`/`weights_uvm` storage, excluding physical padding
  and the empty autograd placeholder. Direct parameters on non-leaf modules are
  also eligible. Checks use bounded chunks, defaulting to 16,777,216 elements
  (`NAN_MODULE_PROBE_CHUNK_ELEMENTS`), rather than a table-sized boolean tensor.
- **Configured and completed coverage are separate.** Startup discovery does
  not prove a weight check executed. After an event-completed snapshot, the
  probe reports cumulative executed modules, inspected weight tensors and
  elements, and TBE buffers inspected versus discovered. It names uninspected
  weight owners, which may alias storage checked through a different owner.
  The scaled embedding tables contain **37,510,618,624 float32 elements**
  (73,262,927 rows × 512); dense parameters are additional. Verify actual TBE
  coverage before using a clean run to make any statement about embedding
  weights.
- **Training forwards are explicitly scoped.** With the probe enabled, the
  trainer invokes `model(...)` so root hooks run. It pauses hooks after the
  forward snapshot and reactivates them with the next training step. Evaluation
  and backward recomputation therefore cannot be attributed to a stale forward
  step. Optional `NAN_MODULE_PROBE_SHAPES=1` logs the first 12 steps' KJT shapes,
  batch size, and already-cached per-feature value counts without computing
  missing length caches.
- **Gaps and failures are explicit.** Lazy awaitables are not materialized:
  probing their attributes can call a blocking `wait()`, so they are marked as
  incomplete traversal. Opaque objects and excessive nesting are also flagged.
  CPU floating tensors disable the probe with a diagnostic instead of adding a
  host/device reduction transfer. Hook failures remove the hooks. Optimizer
  state is not inspected. `_compute_loss` remains covered through its enclosing
  `DefaultMultitaskModule.forward`, although the free function has no hook of
  its own.

**Interpretation limits.** An absent leaf candidate does not establish a fused
kernel defect: prior weight corruption or a coverage gap can explain it. A clean
probe run covers the outputs and weights actually inspected during that run;
it cannot establish the uninstrumented failure rate or exclude a later onset.
Full embedding scans materially perturb the workload, so retain a matching
uninstrumented control. The focused self-test exercises first-call state,
keyword-only inputs, existing bad parameters, hidden TBE lookups, delayed host
snapshots, bounded chunks, lazy awaitables, and pause/reactivation behavior.
...
**Full-state capture now exists.** The legacy hook did not obtain complete
adjacent replay boundaries and skips large/fused state. The new recorder saved
pre-step 100 and 101, including inputs and every embedding byte. It does not
retain all intermediate operation tensors; the new backward-boundary probe
captures those on the first extreme/nonfinite call. See [2.7](#27-full-state-capture-and-replay-2026-09-18).

### 2.5 What runs next

1. **Preserve the failure artifact off-host**, together with its exact source,
   stack identity and checksum manifest. See [2.8](#28-capture-backup-and-retention).
   A local capture is not yet a backup.
2. **Compare the saved operation tensors and their CPU references.** The second
   AC cycle restored compute health; four direct attempts completed. The first
   pair's layer-1 `duvqk` maxima differed by 12,841× before the GEMM. The fully
   verified, save-all pair has byte-identical GEMM inputs/outputs. Two isolated LN
   repeats match the capture; a 16-row FP64 control also agrees after BF16 cast.
   An anomalous operation dump and reference comparison remain needed. See the
   [second-AC report](nan_replay_post_ac_20260918_0730.md).
3. **Separate numerical replay from execution-history reproduction.** Compare
   one attempt per fresh process with same-process repeats, and step 100 → 101
   replay. Verify restored bytes, controls and RNG each time. Save-all and extra
   verification changed the observed divergence; neither is a kernel fix. Keep
   the paired image, buffer-op disable, BLOCK_N=64 and pipeline clamp unchanged.
4. **Follow the first anomalous boundary upstream.** Extreme `duvqk` entering
   the GEMM points to attention/SiLU or earlier state. Ordinary GEMM inputs with
   extreme output require an isolated GEMM replay and CPU FP64 comparison. For
   layer norm, compare the FP64 reference against the output dtype's range:
   valid overflow from an extreme input does not identify a layer-norm defect.
5. **If direct replay stays finite, test preceding-step replay and bounded
   repeats.** Report intermediate state/RNG differences. Small healthy backward
   nondeterminism is already measured; allocator history and timing are not
   captured. If necessary, run the same full-state recorder with earlier
   operation probes, preserving the original artifact.
6. **Produce a component reproducer and a controlled A/B.** Preserve original
   inputs, layouts, controls and observed outputs. A candidate fix must repair
   that reproduction, then pass the original model replay and longer same-stack
   training. Onset distributions alone neither identify a component nor prove
   that NaN and hang share a cause.

**What `START_TS=0` changes.** The early daily windows contain few anchors, but
`drop_last=True` discards incomplete batches: the window map in
[2.1](#21-the-ts0-nan-nondeterministic-onset-step-5-11) includes entire windows
that contribute no step. This does not imply an effective batch below 1024 or
a mismatched `AUTOTUNE_B` entry on steps that execute. The saved old
step-10/11/12 batches all have **1024 samples**.

`MIN_HISTORY=4086` counts raw prior events for anchor eligibility; the default
interleaved history builder then filters events into three behavior pools and
caps each at 1362 without reallocating unused capacity. Selected UIH rows are
therefore **not full-length by construction**. Those three saved batches have
**1697 history events per sample**, totaling **1,737,728 UIH rows** per history
feature. Step 10 contains only one user and one candidate item, with just 104
unique item IDs among those 1,737,728 history indices. This points to a useful
duplication-heavy embedding test case, not a proven culprit; see
[the source and capture review](nan_source_review_20260918.md).

For new runs, record the effective batch, selected-history lengths, and actual
jagged `N` through the early onset region before choosing a shape-based A/B.
The old captures establish that partial batches and fixed 4086-event histories
are both incorrect descriptions of the observed steps. They are already after
NaN onset and cannot identify which component first produced the bad value.

The broader, non-`nan` re-test queue is unchanged and lives in
[6.1](#61-re-test-queue).

### 2.6 The ts=0 hang (2026-09-18)

This section retains the **original step-211 hang**. An AC cycle subsequently
allowed the finite retests and the new captures in [2.7](#27-full-state-capture-and-replay-2026-09-18).
The later replay hang is separately recorded there; pending-recovery statements
inside the following historical account describe the earlier boot state.

> **03:20 UTC follow-up: the stuck component is now sampled.** This was an
> inspection of the **existing** step-211 hang, not an independent repeat.
> System-wide kernel `perf` sampling (`--synth no`, avoiding the hung task's
> memory-map locks) captured **245/245 target-thread samples** in
> `mes_v12_1_submit_pkt_and_poll_completion`, called by
> `mes_v12_1_inv_tlbs_pasid` through `amdgpu_gem_va_ioctl`.
> The host had `halt_if_hws_hang=1`: the driver deliberately loops in
> `while (halt_if_hws_hang) schedule();` after a MES completion timeout.
> **This supersedes the “not MES” interpretation below.** The blocked ioctl is
> the AMDGPU DRM virtual-address mapping path; the earlier KFD WAIT_EVENTS log
> traffic did not identify the blocked thread.
>
> Setting the writable debug parameter to `0` released the loop and let the
> pending kill advance into process teardown. It **did not restore GPU health**:
> teardown still waits in MES/GART TLB handling, and a fresh `probe_stack.py`
> invocation did not complete initialization. New dmesg evidence includes
> `INVALIDATE_TLBS`, `REMOVE_QUEUE`, and `WAIT_REG_MEM` timeouts.
> The live setting is now **0**, with no persistent configuration edit.
> Fresh training repeats require device recovery; a shared-host reboot is
> pending user approval. No new NaN hit rate is claimed.
>
> Triage owner: **AMDGPU/MES firmware and GPU virtual-memory handling**.
> The initial MES timeout trigger, and any connection to Triton numerical
> corruption, remain unproven. See [the triage handoff](hang_triage_20260918.md)
> for evidence, exact stack, and the recovery/test plan.


**One run, on the image/Triton pairing in [3.1](#31-pins).** `START_TS=0`,
batch 1024, `EMBEDDING_ROW_SCALE=0.25`, container
`triton-7ff97e-20260910`. It reached **step 211 with zero non-finite loss** and
then stopped advancing. Log:
`~/mi450_logs/hang_ts0_7ff97e_r1_20260918_025004/b1024_ts0_20260910_7ff97e_r1.log`.

| | |
|---|---|
| Steps completed | **211**, `train_loss` 0.15266 → 0.13986, **0 non-finite** |
| Last log write | 02:18:35, at `[lr-warmup] gstep=211` after `[progress] train_batch_idx=8` |
| Rate before stopping | `step_ms=2978.64`, `elapsed_sec=945.1`, 216,064 samples |
| Threads | 676 total — 675 `S`, **1 `R`** (tid 996, main) |
| CPU split | utime 85,590 / stime 180,218 ticks — **stime 2× utime** |
| GPU | GFX 35 % sustained, **856–858 W** (idle band), VRAM **249.6 GB flat** |
| dmesg | repeated `kfd_ioctl 0xc0184b0c`; **no MES errors, no faults, no resets** |
| Driver | fully responsive — `amd-smi` works throughout |

**It is not the documented MES wedge** ([9.1](#91-hardware-and-recovery)): no
`failed to respond to msg=INVALIDATE_TLBS`, no ring-full, no reset, and the
driver never stops answering.

**It is not killable, and that is the most diagnostic fact.**
`scripts/stop_training.sh` sent `SIGKILL` to all 15 container pids and then timed
out — `TIMEOUT after 1209s, VRAM still 249.6 GB`. At 79 minutes the task is still
`State: R` with

```
ShdPnd: 0000000000000102    <- SIGKILL (bit 9) pending
SigBlk: 0000000000000000    <- not blocked
```

A pending, unblocked `SIGKILL` that does not kill means the task never reaches a
signal-check point — it is stuck **inside the kernel**, in a KFD path that does
not return. Reading `/proc/996/stack` also hangs. **This supersedes the earlier
reading in this session that the process was busy-waiting in userspace**; it is
a driver-level stuck state, and the GPU cannot be released without a reboot.

**No native backtrace was obtained.** `gdb -p` attached (`TracerPid` set) but
never finished walking 676 threads' symbols in 10+ minutes and ignored its own
`timeout 100`; it was `SIGKILL`ed and detached cleanly, which did **not** resume
the run. A stale `strace -f -c -p 996` from an earlier probe blocked the first
attempt — it had been attached only 05:09 against a hang already ~30 min old, so
it neither caused nor masked the stall. **Next time use py-spy against the venv
interpreter path, not gdb** — gdb is not usable on this process.

#### Is this the same defect as the `nan`?

**Open. One run cannot decide it.** Both readings are live:

| Same defect | Separate defects |
|---|---|
| The **old stack hung too** — see below | The `nan` was **non-fatal**: `0917d` logged 240 consecutive `nan` steps and kept stepping to a checkpoint save. This hang stalls the pipeline completely |
| Both occur **only at `START_TS=0`** | 211 clean finite steps with **no numerical precursor** — no drift, no warning |
| Both at effectively zero LR (`dense_lr=8.792e-09` at step 211), so **neither is numerical divergence** | Different stop points: the old hangs stop right after a dataloader `window_indices` line; this one stops mid-step |
| Both fit **one misbehaving kernel on data-dependent input** — return garbage → `nan`; never return → hang | |

**The old stack hung too.** Of the five `START_TS=0` runs on stock Triton 3.6.0,
two stopped dead with no crash, no traceback and no exit marker:

| Run | Steps | `nan` | How it ended |
|---|---|---|---|
| `b1024_nantrip_ts0_0917.log` | 0 | 0 | stops mid-log at `window_indices(12)` |
| `b1024_nantrip_ts0_0917c.log` | 1 | 1 | stops mid-log at `window_indices(15)` |
| `b1024_nantrip_ts0_0917d.log` | 250 | 240 | stops after a checkpoint-save line |
| `b1024_capture_ts0_0917e.log` | 57 | 53 | stops after an lr-warmup line |
| `b1024_capture_ts0_0917f.log` | 0 | 0 | stops during startup compile |

So "a ts=0 run stops advancing without dying" **predates the Triton pin**.
**Caveat, and it matters:** none of these logs carries an exit marker either way,
so their tails are equally consistent with a hang and with a hand-issued
`stop_training.sh`. Treat this as suggestive, **not** as established.

#### What would establish a common cause

The post-AC run passed step 211 and later failed with NaN at step 775; the
capture run failed during backward at step 101; direct replay then hung.
These observations supersede the proposal to classify the defect from repeated
step-211 onset alone. Different onset times do not establish a shared mechanism.
The needed evidence is an operation-level reproduction from captured inputs,
plus an A/B that explains both numerical corruption and loss of GPU progress.
The current replay has not provided that evidence.


### 2.7 Full-state capture and replay (2026-09-18)

The objective is to capture the state **before** the first bad step, restore it,
and identify the operation that creates corruption. A finite training run alone
cannot answer that question. Full implementation details and raw evidence paths
are in [nan_replay_20260918.md](nan_replay_20260918.md); completed post-recovery
replays and commands are in the [second-AC report](nan_replay_post_ac_20260918_0730.md).

**The capture does not guarantee one-step NaN reproduction.** It preserves the
model/input/optimizer/RNG state needed to attempt that replay. It does not
preserve the entire GPU/driver state, allocator history or execution timing.
Healthy restoration is verified, but backward is not fully deterministic. The
first actual-failure replay hung; four post-recovery direct attempts were finite.
A repeatable one-step NaN and then an operation-level reproducer remain
experimental results to obtain.

| Experiment / artifact | Outcome | What it establishes |
|---|---|---|
| `nan_long_20260918_0417` | First NaN loss at 775, stopped at 797; no MES/GPU fault | The paired post-AC stack still fails; this run lacks a complete replay state |
| `nan_replay_validate_20260918_0500` and `_0504` | Forced captures at healthy step 3 completed | Full-state persistence works with the actual embedding allocation |
| `_0504/capture_step_000003/replay_report.json` | Two direct and two step-2 → step-3 attempts; exact restored bytes and forward outputs | Replay restores the captured state faithfully; backward updates are not bitwise deterministic |
| `_0504/capture_step_000003/replay_fresh_report.json` | One fresh-input preceding-step replay matches both steps' forward output bytes | Reusing mutated KJT inputs does not explain the healthy replay differences |
| `nan_replay_long_20260918_0520/capture_step_000101` | Complete capture at `backward_before_clip` | The original failure's pre-step 100/101 states are preserved |
| `replay_current_101.log` in that run's log directory | MES hang during first direct replay; exit 143 after SIGTERM | No completed actual-failure replay and no numerical reproduction verdict |
| `nan_replay_post_ac_20260918_0730/replay_report.json` | Two direct attempts finite and forward-exact; `duvqk` maximum changes 12,841× | Divergence is visible before the layer-1 GEMM/LN; repeat 1 restore bytes were not independently verified |
| `nan_replay_post_ac_20260918_0730/saved_operations_report.json` | Two direct attempts finite; each restore byte/control/RNG check exact; all 12,043,722,752 GEMM input/output bytes match | Full verification and complete operation persistence perturb the execution; no fix established |
| `nan_replay_post_ac_20260918_0730/ln_operation_replay.json` | Two complete isolated LN operations match all captured output hashes; 16-row FP64 `d_x` reference matches BF16 cast | Replay works for this finite control; full FP64 parameter reductions and the original bad LN inputs remain untested |
| `nan_backward_boundaries.py` | CPU checks and four real direct replays completed; four finite operation dumps retained | Captures pristine selected GEMM/LN inputs and outputs, including finite controls; original anomaly still lacks an operation dump |

The validation/capture names above are relative to
`/home/chcai/dlrm_data/`; run logs live under `/home/chcai/mi450_logs/`.
The uninstrumented `_0417` name refers to its log directory.

**What the recorder retains.** At each pre-forward boundary, after LR warmup,
`zero_grad()` and input transfer, it retains both batches' KJTs and metadata,
all raw parameters/buffers and hidden TorchRec/TBE storage, all 11 embedding
tables, fused rowwise Adagrad accumulators, dense Adam state, optimizer controls,
iteration counters, Python/NumPy/Torch RNG and effective configuration. The
150,042,474,496-byte embedding backing uses one CPU shadow plus an undo journal
for every changed 16 KiB page. Comparing every raw byte also detects writes
outside the intended embedding rows. Smaller state is copied completely at both
boundaries, preserving aliases, layouts and dynamic scratch shapes.

The recorder saved step 101 because **38 parameter gradients were nonfinite**,
before clipping or the dense optimizer update. Forward loss was approximately
**0.13949690759**, and predictions, labels and weights were finite. Fused
embedding updates can occur within backward; saved pre-step state remains
independent of those updates. The artifact contains pre-step state and bad
gradient names, not the final bad gradient values or all intermediate tensors.

| Backward region | Observed parameter-gradient status |
|---|---|
| HSTU layer 2 | All finite |
| HSTU layer 1 | Input norm weight/bias nonfinite; UVQK and output-side gradients finite |
| HSTU layer 0 and earlier | All seven layer-0 gradients and upstream preprocessing/positional gradients nonfinite |

This pattern makes **layer-1 input-normalization backward and its incoming GEMM**
the first inspection target. It is not proof of a layer-norm defect: the
after-backward scan cannot exclude a later wild store into an earlier gradient.
Also, `triton_addmm_bwd` uses separate **`torch.mm`** calls for weight and input
gradients; a finite weight gradient does not validate the input-gradient GEMM.

**Determinism measured on the healthy capture.** Every restored embedding byte
and small-state byte matched. Direct and preceding-step forward outputs matched
byte for byte. Replaying the preceding backward update changed 9–13 embedding
bytes across the tested attempts; sampled differing weights were one FP32 ULP
apart. The largest rowwise Adagrad difference was `4.33065e-8`; timestamp Adam
moment differences were much smaller. These differences limit a claim of exact
backward determinism and do not themselves explain the NaN.

**The other-host hint changes the check.** A user-provided MI450 screenshot
reports finite incoming gradients up to **6.9e37** and CPU-reference-confirmed
BF16 overflow in layer norm. The actual tensors from that host have not been
inspected here. Our probe therefore stops at finite magnitudes above **`1e20`**
as well as NaN/Inf, saving pristine pre-call inputs, output observations,
magnitudes, counts and affected rows. It does not modify gradients. In this
host's fused path, `d_normed_x` comes directly from `duvqk @ uvqk_weight.T`;
the nearby splits are column views. The other host's jagged-split lead is not
the immediate producer here, though upstream jagged operations remain candidates
if a capture points there.

**Replay hang and instrumentation status.** Automatic replay initially deferred
because the service could not find/access `amd-smi`; its guard now uses the
absolute binary path with the required access. A separate direct replay began
at 06:23:23 after an idle check and encountered real MES failures at 06:25:50.
All 251 sampled userspace locations were in ROCr
`InterruptSignal::WaitRelaxed`; the driver halt flag was already zero. After
SIGTERM, cleanup waited in AMDGPU/MES TLB handling. The replay process eventually
exited 143, without a report. GPU memory drained by 06:57 but compute remained
unverified until the user's second AC cycle around 07:24. A finite Torch matmul
and exact Triton-add health probe then passed, followed by four completed direct
replays. The initial pair's finite pre-GEMM divergence was absent with full
repeat verification and save-all operation dumps. Progress persistence and
SIGUSR1 Python stack dumps are available. See the
[second-AC report](nan_replay_post_ac_20260918_0730.md) for evidence, source audit
limits and exact resume commands; no model-kernel change is claimed as a fix.
At the wrap-up snapshot (**07:53:59 UTC**), no process owned `/dev/kfd`, both
replay services were inactive with exit 0, and GFX/VRAM read **0% / 0.2 GB**.

### 2.8 Capture backup and retention

**Use GitHub for code, reports and the checksum manifest; use off-host object
storage or a backup server for complete captured state.** The failure capture
contains six files totaling **151,508,784,424 bytes (151.509 GB / 141.104 GiB)**.
Its single embedding file is 150,042,474,496 bytes. GitHub blocks ordinary Git
files above **100 MiB**; Git LFS has plan-dependent per-file limits of **2–5 GB**.
Splitting these captures merely to fit GitHub would complicate retention and
restoration. GitHub Releases is also intended for distribution, not the primary
backup of this experiment's very large state.

| Retain | Destination | Purpose |
|---|---|---|
| Capture/replay/probe scripts, trainer integration, reports and source configuration | GitHub repository | Reviewable implementation and reproduction instructions |
| [Step-101 checksum manifest](capture_step_000101_manifest.json) and verified backup location/version | GitHub repository | Identify exact artifact bytes and the off-host copy |
| Entire `capture_step_000101/` directory | Off-host object storage or backed-up NAS/server | Preserve current and preceding replay boundaries |
| Exact source archive/diff, source hashes and run/replay/kernel logs | Same backup location as the capture | Preserve uncommitted capture-time code and the evidence trail |
| Four finite GEMM/LN dumps in `nan_replay_post_ac_20260918_0730/boundaries_saved/` | Same artifact storage; [manifest in Git](evidence/replay_post_ac_20260918_0730/operation_captures_manifest.json) | Preserve complete operation inputs/outputs and the validated replay controls |
| Healthy `_0504/capture_step_000003/` directory and replay reports | Same backup storage, second priority | Preserve a validated control for capture/replay fidelity |
| Future small operation-level reproducer | GitHub if size permits; otherwise artifact storage | Portable component triage without restoring the full model |

All six failure-capture files were read in full and SHA-256 hashed on
2026-09-18; the manifest records paths, exact sizes, hashes and boundaries.
After the second AC cycle, all six were re-read and matched at **07:32:48 UTC**
([verification](evidence/replay_post_ac_20260918_0730/artifact_verification.json)).
**Backup status is currently `local_only`: no remote copy has been uploaded or
verified.** A checksum manifest is an integrity record, not a copy of the tensors.
The required replay unit is `COMPLETE.json`, `context.json`, `capture.pt`,
`state/manifest.json`, `state/storage-000000.bin` and `state/previous.undo`.
Copy them together with relative paths preserved; the undo file is required for
step 100. Parent `rolling/` files are working copies and are not required once
the complete artifact is persisted.

Prioritize the actual failure and its ~17 MB run logs/source package. Keeping
that artifact plus the healthy validation capture requires **302,731,892,133
bytes (302.732 GB / 281.941 GiB)**, before logs and any additional copies.
The four new operation dumps add **36,158,352,092 bytes**, making these three
artifact groups **338,890,244,225 bytes (338.890 GB)** in total. Each operation
dump was read fully and SHA-256 hashed; these tensors also remain local-only.
After transfer, verify every file against the manifest at the destination and
record the destination's object version or snapshot identifier. Retain the
original local capture until that verification succeeds. Preserve the exact
container image in a registry or image archive alongside its recorded image ID;
the state files alone do not recreate the software stack.

GitHub limits checked against its documentation:
[large files](https://docs.github.com/en/repositories/working-with-files/managing-large-files/about-large-files-on-github)
and [Git LFS](https://docs.github.com/en/repositories/working-with-files/managing-large-files/about-git-large-file-storage).
...
| Image | **`recommendation-gfx1250-20260910:triton-7ff97e`** (18 GB), built `FROM amdprimus/amdprimus:gfx1250-20260910` with `7ff97e3109` compiled **into the image** | [`Dockerfile.amdprimus0815`](../../Dockerfile.amdprimus0815) via `--build-arg BASE_IMAGE=` — see [3.1.1](#311-the-imagetriton-pairing-and-how-to-build-it). Older `recommendation-amdprimus0815:gfx1250` + Triton rebuilt in a container is the previous shape and is superseded |
...
#### 3.1.1 The image/Triton pairing, and how to build it

**The image and the Triton pin are orthogonal, and both are load-bearing.** This
is the single easiest thing to get wrong on this host, and it was got wrong once
— silently.

**`amdprimus/amdprimus:gfx1250-20260910` ships stock Triton `3.6.0`, which is too
old for this arch.** A newer *image* is not a newer *Triton* — the same trap
already recorded for `gfx1250-20260831` in [10.1](#101-images-tested). The image
must be paired with a **source build of upstream `7ff97e3109`**.

**Build the Triton into the image, never into a container's writable layer.** A
`pip install -e` done via `docker exec` lives only in that container. `docker rm`
erases it and the next `docker run` of the base image silently reverts to stock
3.6.0, with no error and no version bump visible in the run log. That is exactly
how a prior build was lost. Every fix must survive `docker rm` / `docker run`.

```bash
# Requires recommendation/.dockerignore -- without it the build context is 32 GB
# (a 1.5 GB `core` plus many gpucore.* dumps). With it, 24 MB.
docker build -f Dockerfile.amdprimus0815 \
  ...
  -t recommendation-gfx1250-20260910:triton-7ff97e .
```

The Triton layer is deliberately **last** in the Dockerfile, so re-pinning Triton
does not invalidate the fbgemm build above it. Note `TRITON_COMMIT` must be the
**full 40-char SHA** — GitHub rejects a shallow fetch by abbreviated ref
(`fatal: couldn't find remote ref 7ff97e3109`).

**Verifying the pin — `triton.__version__` is not enough.** It reports bare
`3.8.0` on both the AMD wheel and this source build. The unambiguous checks are
the pip distribution name and the git SHA of the editable tree:

```bash
docker exec <container> pip list | grep -i triton
#   triton          3.8.0+git7ff97e31    /opt/triton-custom     <- the proof
docker exec <container> git -C /opt/triton-custom rev-parse HEAD
#   7ff97e310935b4a79794878dbc911f9af25d38d9
```

**`gfx1250-20260910` ships a broken hipblaslt, and it needs an env var.** A plain
`torch.matmul` dies with `HIPBLAS_STATUS_INVALID_VALUE` and
`hipModuleLoad failed: .../library/Kernels.so-000-gfx1250.hsaco`. The newer
hipblaslt searches the **parent** `library/` directory while the kernels actually
ship in `library/gfx1250/`. Verified on a **fresh container from the image** that
this env var **alone** fixes it, with no symlinks:
...
> The first attempt at this fix was symlinks written into the container's
> writable layer — the same mistake that lost the previous Triton build. Fixing
> a broken image inside a container is never a fix. **This env var is currently
> only in the `docker run` line and is not yet an `ENV` in the Dockerfile**; it
> should be.

**Smoke-test the stack before trusting a run:**
[`scripts/probe_stack.py`](../../scripts/probe_stack.py) checks a 4096² matmul
and a Triton JIT compile+launch on the real interpreter. It exists as a **file**
because Triton 3.8 rejects `@jit` from `python -c`
(`ValueError: @jit functions should be defined in a Python file`). Expected:

```
matmul     : finite=True  0.09s
triton JIT : max_err=0  compile+run=0.9s
```
...
| `HISTORY_LENGTH` / `MIN_HISTORY` / `MAX_SEQ_LEN` | 4086 / 4086 / 4096 | Gin production history budget, anchor-eligibility floor, and model sequence limit. The runner passes no `-e MIN_HISTORY`, so it uses the gin default 4086. **Verified per run:** anchor caches are min_history-keyed (`positions_L<L>.npy` when `MIN_HISTORY == L`, else `positions_L<L>_m<MIN_HISTORY>.npy` — `datasets/yambda.py:331-333`), and the 20 recorded batch-1024 logs open `positions_L4086.npy`, not `_m64`. Eligibility requires 4086 raw prior events; selected UIH length can be smaller because the interleaved builder filters/caps each behavior pool. Old ts=0 step-10/11/12 captures have 1697 selected history events per sample at batch 1024 ([2.5](#25-what-runs-next)) |
...
**Current shape (2026-09-18): Triton lives in the image**
([3.1.1](#311-the-imagetriton-pairing-and-how-to-build-it)), so the container is
disposable and `docker rm` is safe. This is the recipe to use:

```bash
# Create the long-lived container from the paired image. The two -e lines are
# both required: HIPBLASLT_TENSILE_LIBPATH works around the image's broken
# hipblaslt layout (3.1.1), and AMDGCN_USE_BUFFER_OPS=0 is the one correctness
# flag ([1]) -- setting it to 1 wedges the node and costs a reboot.
docker run -d --name triton-7ff97e-20260910 --entrypoint sleep \
  ...
  -e TRITON_CACHE_DIR=/data/mlperf_dlrm_v4/triton_cache_7ff97e \
  ...
  recommendation-gfx1250-20260910:triton-7ff97e infinity

# Verify the stack before any run (3.1.1).
docker exec -w /workspace/recommendation triton-7ff97e-20260910 \
  python scripts/probe_stack.py

# Train at the MLPerf-aligned start point. CONTAINER selects the container; the
# script's SMOKE branch hardcodes START_TS=150, so ts=0 must use the non-SMOKE
# path with START_TS passed explicitly.
CONTAINER=triton-7ff97e-20260910 scripts/run_prod_convergence_1gpu.sh <log> \
  -e START_TS=0
```

> **`NUM_TRAIN_BATCHES` does not bound the run on the non-SMOKE path.** Verified
> empirically on 2026-09-18: a run with `NUM_TRAIN_BATCHES=60` reached step 211
> and was still going. On that path the run terminates only on data exhaustion.
> If you need a bounded ts=0 run, bound it yourself — do not rely on this knob.

> `progress=` in the `[lr-warmup]` line is **warmup** progress against
> `lrw/env_int.default = 24000` ([`yambda_5b.gin:170`](../../generative_recommenders/dlrm_v4/train/gin/yambda_5b.gin)),
> **not** run progress. `progress=0.0088` at step 211 does not mean the run is
> 0.88 % done.

**The older shape, for the archived containers below:** Triton was
`pip install -e`'d **into a container** and produced no image, so it was: start a
long-lived container, build Triton in it, train in it. That is what the
`triton-7ff97e-test` container carries. It is superseded — a container built that
way loses its Triton on `docker rm`.
...
- **A 600-step batch-1024 run takes ~34 min** — ~1.9 min of startup (140 GiB
  ...
  3403 across the two runs; 3492 unclamped). At `START_TS=0` the early steps are
  faster (~1.8 s/step over steps 11-51, on the small warm-up windows) and settle
  to ~3.0 s/step by step 211 — **do not quote the early figure as the rate.**
- **Always stop with [`scripts/stop_training.sh`](../../scripts/stop_training.sh)**
  (container-side kill plus a real VRAM-drain wait), and re-verify the trainer
  count is back to zero before leaving a run unattended. Killing the host-side
  `docker exec` client only detaches it; the trainer keeps running and keeps its
  HBM.
- **`stop_training.sh` cannot stop the ts=0 hang.** Against
  [2.6](#26-the-ts0-hang-2026-09-18) it killed all 15 container pids and then ran
  its drain wait to completion:

  ```
  [stop] killing container pids: 668 995 996 2045 2046 2047 2048 2061 ...
  [stop] waiting for VRAM to drop below 5 GB (timeout 1200s)
  [stop] TIMEOUT after 1209s, VRAM still 249.6 GB
  ```

  pid 996 was still `State: R` at 79 minutes. That is **not a bug in the
  script** — the task is stuck in the kernel below the signal layer, so the
  `SIGKILL` is never delivered. **Recovering the GPU from that state needs a
  reboot**, which on this shared host needs explicit user approval.
- **Do not pipe `stop_training.sh` into `tail`.** The pipeline's exit status is
  `tail`'s, so the script's `exit 1` on drain timeout is masked and the stop
  looks like it succeeded. Piping also buffers all output until the script ends,
  so a 20-minute stop appears to produce nothing. Run it unpiped, or capture with
  `> file 2>&1` and check `${PIPESTATUS[0]}`.
- **`gdb -p` is not usable on the trainer.** It attaches but never finishes
  loading symbols across ~676 threads, and ignores an outer `timeout`. Use py-spy
  against the **venv** interpreter path; py-spy resolving `/usr/bin/python3.12`
  attaches but prints no stacks.
...
> the post-AC 379-step bounded series was finite, while the subsequent long
> run and capture run failed — [1.1](#11-where-this-stands), [2](#2-the-live-nan).
...
- **`recommendation-gfx1250-20260910:triton-7ff97e`** — **the current image**,
  not historical; listed here only so the set is complete. Base
  `amdprimus/amdprimus:gfx1250-20260910`, Triton `3.8.0+git7ff97e31` built
  **into** the image. See [3.1.1](#311-the-imagetriton-pairing-and-how-to-build-it)
  — including the hipblaslt workaround the base image needs. `torch` is
  byte-identical to the `:0815` base, so against the older paired image exactly
  two variables change: the ROCm userspace and Triton.

Base images, for reference: `amdprimus/amdprimus:0815` is what the older repo
image is built `FROM`; `amdprimus/amdprimus:gfx1250-20260831` was evaluated and
rejected — its `libtriton.so` is byte-identical to `:0815`, so a newer image is
not a newer Triton; and `amdprimus/amdprimus:gfx1250-20260910` is the current
base, which **also ships stock Triton 3.6.0** and must be paired with a source
build. The lesson generalises: **check the Triton the image actually carries,
every time.**
...
- `scripts/nan_replay_storage.py`, `nan_replay_state.py`,
  `nan_replay_capture.py` and the trainer integration in
  `generative_recommenders/dlrm_v4/train/utils.py` — complete adjacent pre-step
  state/input capture, raw embedding-byte comparison and undo, optimizer/RNG
  restoration, and stop on nonfinite state/output/gradients before clipping.
- `scripts/replay_training_step.py` — current or preceding-step replay,
  restoration-byte checks, fresh KJTs, output/state/RNG comparisons, backward
  gradient scans, optional boundary capture and persistent progress reports;
  `--verify-every-repeat` verifies large/small bytes, controls and RNG each time.
- `scripts/nan_backward_boundaries.py` — capture pristine inputs at the suspect
  GEMM/layer-norm backward boundaries; detect extreme finite values as well as
  NaN/Inf. `NAN_BACKWARD_SAVE_ALL=1` also retains finite selected-operation
  inputs/outputs. Validated on four actual step-101 direct replays.
- `scripts/replay_backward_boundary.py` — bounded CPU byte comparison and FP64
  references for captured GEMM/LN operations; optional complete GPU-operation
  replay with source/stack checks and fresh preserved-layout inputs. Saved GEMM
  pairs compared fully; isolated LN replay validated twice against full-output
  hashes and a 16-row FP64 `d_x` control.
- `scripts/test_nan_replay.py` — retained CPU regressions for snapshot undo,
  aliases/dynamic storage and optimizer/RNG restoration. Additional probe mocks
  were run separately; they are not all part of this test file.
- `scripts/monitor_mi450_nan_run.py` — loss/progress, first-failure and GPU-fault
  evidence monitor for the long-run experiments.
- `docs/mi450_a0/nan_replay_20260918.md` and
  `capture_step_000101_manifest.json` — experiment record, replay limitations,
  backup instructions and full SHA-256 inventory of the actual failure capture.
- `docs/mi450_a0/nan_replay_post_ac_20260918_0730.md` and
  `evidence/replay_post_ac_20260918_0730/` — second-AC findings, source audit
  limits, exact resume commands, and small reports/logs/launchers. Full captured
  model state and operation tensor dumps remain outside Git and locally stored.
