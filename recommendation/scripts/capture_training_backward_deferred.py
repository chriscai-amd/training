#!/usr/bin/env python3
"""Uncapped deferred scalar-only trace of all three layers, resolved before clipping."""
...
VARIANT = "uncapped_deferred_all3layers_all7ops_preclip_v1"
...
def configure_environment(options, environment):
    ...
    recorded = {**options, "target": "*", "operations": list(ALL_OPERATIONS),
                ...
                "dense_phase_sentinels": True, "deferred_scalar_trace": True, "raw_snapshot_present": False}
...
def diagnostic_worker(local_rank, world_size, node_rank, gpus_per_node,
    ...
    if (options.get("monitor_variant") != VARIANT or options.get("attention_vgpr_cap") != 0
            ...
            or options.get("deferred_scalar_trace") is not True
            or options.get("raw_snapshot_present") is not False):
    ...
    try:
        ...
        if error.code == 42 and path.is_file() and json.loads(path.read_text()).get("status") == "bounded_complete":
            print(f"[deferred-training-boundary] completed requested {options['steps']} steps: {path}", flush=True)
            return
        raise
    finally:
