...
everything runnable without a dataset — the two `scripts/repro_gfx1250_*.py`
scripts, the unit tests, and the compile-only checks. Both cells are always
filled, so a "none" is stated rather than left to inference; three defects have no
...
Triton wheel — pick one with `IMG=`. The two `scripts/repro_gfx1250_*.py` scripts
need **only torch and Triton** (no repo imports, no fbgemm, no dataset), so those
are the ones to hand over.
...
Before the port on this branch, the same `7ff97e3109` build failed all eight
`attn_check.py` cases at the first Triton forward because `tl.make_block_ptr`
raises `NotImplementedError`.  HSTU attention now uses masked plain-pointer
loads instead, preserving physical V offsets separately from the remapped
contextual/multi-target mask coordinates.  All eight deterministic BF16 cases
pass forward and backward at the original `atol=1e-5`, `rtol=0.016` on both
...
The linear and layer-norm kernels still contain 22
`tl.make_block_ptr`/`tl.advance` calls, so an end-to-end latest-main run remains
blocked until those are ported too.  The existing descriptor/TLX path is
NVIDIA Hopper-specific and deliberately disabled on HIP.  Keep
`AMDGCN_USE_BUFFER_OPS=0`; the independent buffer-addressing hang has not been
cleared on latest main.
