...
**What breaks it.** Each row is one change from the recipe above, measured on
this stack. The first is the dangerous one: the other three are debug knobs you
would leave alone, but batch size is an ordinary configuration choice. The last
column is what a Triton-side triage can run — **only two of the four have one**.

| Change | Symptom | Defect | Standalone reproducer for triage |
|---|---|---|---|
| **`BATCH_SIZE=128`** | `train_loss` **`nan` by step 50**, never recovers. Three batch-128 runs agree; all four batch-8 runs are clean | [3] | **None.** E2e only, and it still needs the dataset — but it is now ~50 steps instead of 2200, so the cost of a hypothesis is minutes. Owed: bisect to a kernel with `DEBUG_NAN_HOOKS=1`, which names the kernel that *manufactured* the first non-finite gradient |
| `TRITON_ALLOW_PIPELINING=1` | `nan` at step 100 at batch 8, against a matched control that stays finite. No fault, no garbage | [4] | **None on this stack**, and this is the worst gap. [`repro_gfx1250_pipeliner.py`](../scripts/repro_gfx1250_pipeliner.py) is torch+triton only but calls `tl.make_block_ptr` in 6 places, so on `7ff97e3109` it dies with [5]'s removal error before testing anything; and `attn_check.py` now **passes** at `num_stages=2`, so attention is no longer a witness. Owed: a plain-pointer rewrite of the probe, aimed at a *non-attention* kernel (see below) |
...
- **[3] `train_loss=nan` — reproducible, and it is a batch-size effect.** The
  owed batch-128 run (`results/nan_probe_b128/`) goes `nan` at the first logged
  point, **step 50** — not step 2200 as on AMD 3.7.1. Three batch-128 runs
  agree, four batch-8 runs are clean, so the split is batch size, not step
  count. That is a ~50-step reproducer where there was none. Not yet bisected to
  a kernel.
- **[4] pipelining is not fixed upstream — it changed symptom.** With
  ...
  AMD 3.8.0 returned `3.3e+28`. But e2e goes `nan` at step 100 against a matched
  control (`results/pipelining_probe/` vs `pipelining_control/`). **Row 3's
  clamp stays load-bearing**, and the attention-only check would have wrongly
  cleared it. **Lead on [3]:** the clamp only applies on Triton ≥ 3.8, so AMD
  3.7.1 — where [3] first appeared — ran unclamped, and pipelining `nan` is the
  same symptom. But the clamp is already on in the batch-128 run, so if they are
  one defect the clamp does not cover that path.
