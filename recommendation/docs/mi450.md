...
- **Current main under test:** `triton-lang/triton` @
  ...
  pointer port, and completes e2e once the attention backward is pinned to
  `BLOCK_N=64` on gfx1250 ([6]) — 4000 batches, `train_loss=0.10000`, AUC
  `0.5233`.
...
Compile is therefore clear on `7ff97e3109` ([5] gone). The e2e failure that was
left is table row 10 / defect [6]: a `Page not present` memory-access fault
...
fault is. It is now worked around; see "Working around defect [6]" below.
...
iteration and it faults within ~150 iterations, with no dataset, no TBE and no
TorchRec:
...
### Working around defect [6]

The trigger is **register pressure at the pinned backward tile**, not anything
in the jagged layout itself — the varying layout only decides which
specialization gets compiled. `_get_bw_pinned_configs()` pinned
`BLOCK_M=32, BLOCK_N=128` on HIP, and `pinned_or_full()` makes that the *only*
config unless `TRITON_FULL_AUTOTUNE=1`, so it is exactly what the backward runs.
Reading `.vgpr_count` out of the compiled AMDGCN for both tiles:
...
Two of the 128 specializations sit exactly on the gfx1250 1024-VGPR ceiling and
spill to scratch; the 64 tile never comes close and never spills. Pinning
`BLOCK_N=64` on gfx1250 + Triton ≥ 3.8
([PR #7](https://github.com/chriscai-amd/training/pull/7), `BLOCK_N=128`
retained everywhere else) clears every reproducer for this defect:

- `repro_gfx1250_attn_bwd.py --part bwd` — 4000/4000 on `--layout contiguous`,
  `--layout strided`, `--layout guarded`, and `--contextual 0 --no-targets`,
  where the same commands faulted by step ~125–150 at `BLOCK_N=128`.
- e2e, 4000 batches at `EMBEDDING_ROW_SCALE=0.25`, batch 8,
  `HSTU_HAMMER_KERNEL=TRITON` — 5.2 min to `run_stop`, `train_loss=0.10000`
  against the PyTorch-kernel reference `0.1015`, eval AUC `0.5233`, no `nan`.
  That last point is not yet evidence about [3]: 4000 × batch 8 is 32,000
  samples, and [3]'s 3.7.1 onset was at 281,600 (step 2200 at batch 128).

This is a workaround. Spilling is legal codegen, so the defect that computes a
bad store address under it is still in Triton/LLVM, and any shape that pushes
this kernel back to 1024 VGPRs can hit it again — `TRITON_FULL_AUTOTUNE=1`
restores the full config list and can pick a 128 tile. Two details worth
knowing: the gate calls `torch.cuda.get_device_properties(0)` at import time,
which initialises a HIP context on device 0 before a spawned rank picks its own,
and `>= (3, 8)` also matches ROCm's `3.8.0+git4cff872c`, so row 8 / defect [4d]
— the same kernel — takes this change too, untested as of this writing.
...
version-specific fork. `scripts/attn_check.py` is the numeric check that the
attention port matches the previous kernels on 3.8.0 and on `7ff97e3109`.
