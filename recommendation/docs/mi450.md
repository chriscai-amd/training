...
would leave alone, but batch size is an ordinary configuration choice.

| Change | Symptom | Defect |
|---|---|---|
| **`BATCH_SIZE=128`** | `train_loss` **`nan` by step 50**, never recovers. Three batch-128 runs agree; all four batch-8 runs are clean | [3] |
| `TRITON_ALLOW_PIPELINING=1` | `nan` at step 100 at batch 8, against a matched control that stays finite. No fault, no garbage | [4] |
| `AMDGCN_USE_BUFFER_OPS=1` | **Node hang, reboot required** | [1] |
| `TRITON_FULL_AUTOTUNE=1`, or reverting the pin to `BLOCK_N=128` | `Memory access fault … Page not present` within a few hundred batches; recoverable | [6] |
