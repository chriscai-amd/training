...
`docker commit triton-7ff97e <tag>` then `IMG=<tag> NUM_TRAIN_BATCHES=4000
./scripts/run_smoke_shrink_1gpu.sh` is the same run through the runner — where
those defaults come from — at the cost of a ~47 GB layer.
